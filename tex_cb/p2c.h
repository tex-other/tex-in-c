...
#define ligkernstart(x,y) (ligkernbase[(x) - fontbase] + rembyte(y))
#define ligkernrestart(x,y) (ligkernbase[(x) - fontbase] + opbyte(y) * 256 + rembyte(y) - kernbaseoffset + 32768L)
#define  exteninfo(f,q) (fontinfo[extenbase[(f) - fontbase] + rembyte(q)].qqqq)

#define freeavail(x) (mem[(x)].hh.rh=avail,avail=(x),dynused--)
#define fastgetavail(x) (((x)=avail)?(avail= *flink(x), *flink(x)=0,dynused++):\
			((x)=getavail()))
...
#define storenewtoken(p,x)   { int q=getavail();*flink(p)=q,*finfo(q)=(x),p=q;}
#define faststorenewtoken(p,x)   {int q;fastgetavail(q), \
...
*/
#define set_as_char_node(x) 0 /*(mem[(x)].is_char_node=1)*/
