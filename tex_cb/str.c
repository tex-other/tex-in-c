...
#define POOLPOINTER_IS_POINTER 0
...
void str_room(long l)
{
  if (poolptr + l > pool_top)
    overflow(S(1277), pool_top - initpoolptr);
}
...
#if 0
		int dif=strpool[j+l]-strpool[k+l];
#else
...
int str_bcmp(unsigned char * buffp, long l,strnumber s)
{
      if (flength(s) == l) {
#if 0
	if (!memcmp(strpool+strstart[s],buffp,l))
#else
