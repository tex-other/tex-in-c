...
#undef halfword
#define halfword move_p
#undef pointer
#define pointer move_p
...
Static move_p getnode(long s)
{
	move_p pom=(move_p)malloc(sizeof(* pom));
	if(pom) { 	
		return pom;
	} else { 
...
void dvifontdef(internalfontnumber f)
{
  ...
  dviout(fontcheck[f ].b3 - minquarterword);
  dvifour(fontsize[f ]);
  dvifour(fontdsize[f ]);
  dviout(0);
  dviout(flength(fontname[f ]));
  str_map(fontname[f ],dviout_helper);
}
/*:602*/
...
void movement(long w, eightbits o)
{
  ...
  pointer q = getnode(movementnodesize);
  width(q) = w;
