...
Static void endname(void)
{
  ...
  if (strptr + 3 > maxstrings)
    overflow(S(665), maxstrings - initstrptr);
  if (areadelimiter == 0)
    curarea = S(385);
  else {
    curarea = strptr;
...
Static void startinput(void)
{
  ...
  while (true) {
    ...
    if (aopenin(&curfile))
      goto _Ldone;
    if (curarea == S(385)) {
      packfilename(curname, S(678), curext);
      if (aopenin(&curfile))
	goto _Ldone;
    }
    endfilereading();
    promptfilename(S(666), S(670));
  }
_Ldone:
...
Static void scanspec(groupcode c, boolean threecodes)
{
  ...
  if (threecodes)
    s = saved(0);
  if (scankeyword(S(698)))
    speccode = exactly;
  else if (scankeyword(S(699)))
    speccode = additional;
  else {
    speccode = additional;
    curval = 0;
...
Static halfword finiteshrink(halfword p)
{
  ...
  if (noshrinkerroryet) {
    ...
    printnl(S(292));
    print(S(749));
    help5(S(750),S(751),S(752),S(753),S(754));
    error();
  }
  q = newspec(p);
...
Static quarterword newtrieop(smallnumber d, smallnumber n, quarterword v)
{
  ...
  while (true) {
	...
	overflow(S(768), trieopsize);
      u = trieused[curlang];
      if (u == maxquarterword)
	overflow(S(769), maxquarterword - minquarterword);
      trieopptr++;
      u++;
      trieused[curlang] = u;
...
Static void offsave(void)
{
  ...
  print(S(555));
  inslist(link(temphead));
  help5(S(839),S(840),S(841),S(842),S(843));
  error();
}
...
Static void extrarightbrace(void)
{
  ...
  help5(S(845),S(846),S(847),S(848),S(849));
  error();
  alignstate++;
}
...
Static void alignerror(void)
{
  if (labs(alignstate) > 2) {   /*1128:*/
    ...
    printcmdchr(curcmd, curchr);
    if (curtok == tabtoken + '&') {
      help6(S(887),S(888),S(889),S(890),S(891),S(892));
    } else {
      help5(S(887),S(893),S(890),S(891),S(892));
    }
    error();
    return;
  }
...
Static void scandelimiter(halfword p, boolean r)
{
  ...
  if (curval < 0) {   /*1161:*/
    printnl(S(292));
    print(S(906));
    help6(S(907),S(908),S(909),S(910),S(911),S(912));
    backerror();
    curval = 0;
  }
  /*:1161*/
...
_Lrestart:
  ...
  printnl(S(292));
  print(S(936));
  help5(S(937),S(938),S(939),S(940),S(941));
  if (curcs == 0)
    backinput();
...
_Lcommonending:
  if (interaction < errorstopmode) {
    ...
    help3(S(984),S(985),S(986));
  } else {
    help5(S(984),S(985),S(986),S(987),S(988));
  }
  error();
}
...
/* p2c: tex1.p: Note: Using % for possibly-negative arguments [317] */
  ...
  while (!wopenout(&fmtfile))
    promptfilename(S(992), formatextension);
  printnl(S(993));
...
/*
  ...
  /*:226*/
  /*230:*/
  primitive(S(1027), assigntoks, outputroutineloc);
  primitive(S(1028), assigntoks, everyparloc);
  primitive(S(1029), assigntoks, everymathloc);
  primitive(S(1030), assigntoks, everydisplayloc);
  primitive(S(1031), assigntoks, everyhboxloc);
  primitive(S(1032), assigntoks, everyvboxloc);
  primitive(S(1033), assigntoks, everyjobloc);
  primitive(S(1034), assigntoks, everycrloc);
  primitive(S(1035), assigntoks, errhelploc);   /*:230*/
  /*238:*/
  primitive(S(1036), assignint, intbase);
  primitive(S(1037), assignint, intbase + tolerancecode);
  primitive(S(1038), assignint, intbase + linepenaltycode);
  primitive(S(1039), assignint, intbase + hyphenpenaltycode);
  primitive(S(1040), assignint, intbase + exhyphenpenaltycode);
  primitive(S(1041), assignint, intbase + clubpenaltycode);
  primitive(S(1042), assignint, intbase + widowpenaltycode);
  primitive(S(1043), assignint, intbase + displaywidowpenaltycode);
  primitive(S(1044), assignint, intbase + brokenpenaltycode);
  primitive(S(1045), assignint, intbase + binoppenaltycode);
  primitive(S(1046), assignint, intbase + relpenaltycode);
  primitive(S(1047), assignint, intbase + predisplaypenaltycode);
  primitive(S(1048), assignint, intbase + postdisplaypenaltycode);
  primitive(S(1049), assignint, intbase + interlinepenaltycode);
  primitive(S(1050), assignint, intbase + doublehyphendemeritscode);
  primitive(S(1051), assignint, intbase + finalhyphendemeritscode);
  primitive(S(1052), assignint, intbase + adjdemeritscode);
  primitive(S(1053), assignint, intbase + magcode);
  primitive(S(1054), assignint, intbase + delimiterfactorcode);
  primitive(S(1055), assignint, intbase + loosenesscode);
  primitive(S(1056), assignint, intbase + timecode);
  primitive(S(1057), assignint, intbase + daycode);
  primitive(S(1058), assignint, intbase + monthcode);
  primitive(S(1059), assignint, intbase + yearcode);
  primitive(S(1060), assignint, intbase + showboxbreadthcode);
  primitive(S(1061), assignint, intbase + showboxdepthcode);
  primitive(S(1062), assignint, intbase + hbadnesscode);
  primitive(S(1063), assignint, intbase + vbadnesscode);
  primitive(S(1064), assignint, intbase + pausingcode);
  primitive(S(1065), assignint, intbase + tracingonlinecode);
  primitive(S(1066), assignint, intbase + tracingmacroscode);
  primitive(S(1067), assignint, intbase + tracingstatscode);
  primitive(S(1068), assignint, intbase + tracingparagraphscode);
  primitive(S(1069), assignint, intbase + tracingpagescode);
  primitive(S(1070), assignint, intbase + tracingoutputcode);
  primitive(S(1071), assignint, intbase + tracinglostcharscode);
  primitive(S(1072), assignint, intbase + tracingcommandscode);
  primitive(S(1073), assignint, intbase + tracingrestorescode);
  primitive(S(1074), assignint, intbase + uchyphcode);
  primitive(S(1075), assignint, intbase + outputpenaltycode);
  primitive(S(1076), assignint, intbase + maxdeadcyclescode);
  primitive(S(1077), assignint, intbase + hangaftercode);
  primitive(S(1078), assignint, intbase + floatingpenaltycode);
  primitive(S(1079), assignint, intbase + globaldefscode);
  primitive(S(333), assignint, intbase + curfamcode);
  primitive(S(1080), assignint, intbase + escapecharcode);
  primitive(S(1081), assignint, intbase + defaulthyphencharcode);
  primitive(S(1082), assignint, intbase + defaultskewcharcode);
  primitive(S(1083), assignint, intbase + endlinecharcode);
  primitive(S(1084), assignint, intbase + newlinecharcode);
  primitive(S(1085), assignint, intbase + languagecode);
  primitive(S(1086), assignint, intbase + lefthyphenmincode);
  primitive(S(1087), assignint, intbase + righthyphenmincode);
  primitive(S(1088), assignint, intbase + holdinginsertscode);
  primitive(S(1089), assignint, intbase + errorcontextlinescode);
  /*:238*/
  /*248:*/
  primitive(S(1090), assigndimen, dimenbase);
  primitive(S(1091), assigndimen, dimenbase + mathsurroundcode);
  primitive(S(1092), assigndimen, dimenbase + lineskiplimitcode);
  primitive(S(1093), assigndimen, dimenbase + hsizecode);
  primitive(S(1094), assigndimen, dimenbase + vsizecode);
  primitive(S(1095), assigndimen, dimenbase + maxdepthcode);
  primitive(S(1096), assigndimen, dimenbase + splitmaxdepthcode);
  primitive(S(1097), assigndimen, dimenbase + boxmaxdepthcode);
  primitive(S(1098), assigndimen, dimenbase + hfuzzcode);
  primitive(S(1099), assigndimen, dimenbase + vfuzzcode);
  primitive(S(1100), assigndimen,
	    dimenbase + delimitershortfallcode);
  primitive(S(1101), assigndimen,
	    dimenbase + nulldelimiterspacecode);
  primitive(S(1102), assigndimen, dimenbase + scriptspacecode);
  primitive(S(1103), assigndimen, dimenbase + predisplaysizecode);
  primitive(S(1104), assigndimen, dimenbase + displaywidthcode);
  primitive(S(1105), assigndimen, dimenbase + displayindentcode);
  primitive(S(1106), assigndimen, dimenbase + overfullrulecode);
  primitive(S(1107), assigndimen, dimenbase + hangindentcode);
  primitive(S(1108), assigndimen, dimenbase + hoffsetcode);
  primitive(S(1109), assigndimen, dimenbase + voffsetcode);
  primitive(S(1110), assigndimen, dimenbase + emergencystretchcode);
  /*:248*/
  /*265:*/
  primitive(' ', exspace, 0);
  primitive('/', italcorr, 0);
  primitive(S(417), accent, 0);
  primitive(S(1111), advance, 0);
  primitive(S(1112), afterassignment, 0);
  primitive(S(1113), aftergroup, 0);
  primitive(S(1114), begingroup, 0);
  primitive(S(1115), charnum, 0);
  primitive(S(262), csname, 0);
  primitive(S(1116), delimnum, 0);
  primitive(S(1117), divide, 0);
  primitive(S(263), endcsname, 0);
  primitive(S(837), endgroup, 0);
  text(frozenendgroup) = S(837);
  eqtb[frozenendgroup - activebase] = eqtb[curval - activebase];
  primitive(S(1118), expandafter, 0);
  primitive(S(1119), deffont, 0);
  primitive(S(1120), assignfontdimen, 0);
  primitive(S(725), halign, 0);
  primitive(S(864), hrule, 0);
  primitive(S(1121), ignorespaces, 0);
  primitive(S(374), insert_, 0);
  primitive(S(402), mark_, 0);
  primitive(S(914), mathaccent, 0);
  primitive(S(1122), mathcharnum, 0);
  primitive(S(404), mathchoice, 0);
  primitive(S(1123), multiply, 0);
  primitive(S(898), noalign, 0);
  primitive(S(1124), noboundary, 0);
  primitive(S(1125), noexpand, 0);
  primitive(S(388), nonscript, 0);
  primitive(S(901), omit, 0);
  primitive(S(462), setshape, 0);
  primitive(S(762), breakpenalty, 0);
  primitive(S(949), setprevgraf, 0);
  primitive(S(416), radical, 0);
  primitive(S(656), readtocs, 0);
  primitive(S(1126), relax, 256);
  text(frozenrelax) = S(1126);
  eqtb[frozenrelax - activebase] = eqtb[curval - activebase];
  primitive(S(971), setbox, 0);
  primitive(S(604), the, 0);
  primitive(S(463), toksregister, 0);
  primitive(S(403), vadjust, 0);
  primitive(S(1127), valign, 0);
  primitive(S(415), vcenter, 0);
  primitive(S(1128), vrule, 0);   /*:265*/
  /*334:*/
  primitive(S(761), parend, 256);
  parloc = curval;
  partoken = cstokenflag + parloc;   /*:334*/
  /*376:*/
  primitive(S(1129), input, 0);
  primitive(S(1130), input, 1);   /*:376*/
  /*384:*/
  primitive(S(1131), topbotmark, topmarkcode);
  primitive(S(1132), topbotmark, firstmarkcode);
  primitive(S(1133), topbotmark, botmarkcode);
  primitive(S(1134), topbotmark, splitfirstmarkcode);
  primitive(S(1135), topbotmark, splitbotmarkcode);   /*:384*/
  /*411:*/
  ...
  /*416:*/
  primitive(S(1136), setaux, hmode);
  primitive(S(1137), setaux, vmode);
  primitive(S(1138), setpageint, 0);
  primitive(S(1139), setpageint, 1);
  primitive(S(1140), setboxdimen, widthoffset);
  primitive(S(1141), setboxdimen, heightoffset);
  primitive(S(1142), setboxdimen, depthoffset);
  primitive(S(1143), lastitem, intval);
  primitive(S(1144), lastitem, dimenval);
  primitive(S(1145), lastitem, glueval);
  primitive(S(1146), lastitem, inputlinenocode);
  primitive(S(1147), lastitem, badnesscode);   /*:416*/
  /*468:*/
  primitive(S(1148), convert, numbercode);
  primitive(S(1149), convert, romannumeralcode);
  primitive(S(1150), convert, stringcode);
  primitive(S(1151), convert, meaningcode);
  primitive(S(1152), convert, fontnamecode);
  primitive(S(1153), convert, jobnamecode);   /*:468*/
  /*487:*/
  primitive(S(658), iftest, ifcharcode);
  primitive(S(1154), iftest, ifcatcode);
  primitive(S(1155), iftest, ifintcode);
  primitive(S(1156), iftest, ifdimcode);
  primitive(S(1157), iftest, ifoddcode);
  primitive(S(1158), iftest, ifvmodecode);
  primitive(S(1159), iftest, ifhmodecode);
  primitive(S(1160), iftest, ifmmodecode);
  primitive(S(1161), iftest, ifinnercode);
  primitive(S(1162), iftest, ifvoidcode);
  primitive(S(1163), iftest, ifhboxcode);
  primitive(S(1164), iftest, ifvboxcode);
  primitive(S(1165), iftest, ifxcode);
  primitive(S(1166), iftest, ifeofcode);
  primitive(S(1167), iftest, iftruecode);
  primitive(S(1168), iftest, iffalsecode);
  primitive(S(1169), iftest, ifcasecode);   /*:487*/
  /*491:*/
  primitive(S(1170), fiorelse, ficode);
  text(frozenfi) = S(1170);
  eqtb[frozenfi - activebase] = eqtb[curval - activebase];
  primitive(S(664), fiorelse, orcode);
  primitive(S(1171), fiorelse, elsecode);   /*:491*/
  /*553:*/
  primitive(S(1172), setfont, nullfont);
  text(frozennullfont) = S(1172);
  eqtb[frozennullfont - activebase] = eqtb[curval - activebase];   /*:553*/
  /*780:*/
  primitive(S(1173), tabmark, spancode);
  primitive(S(738), carret, crcode);
  text(frozencr) = S(738);
  eqtb[frozencr - activebase] = eqtb[curval - activebase];
  primitive(S(1174), carret, crcrcode);
  text(frozenendtemplate) = S(1175);
  text(frozenendv) = S(1175);
  ...
  /*983:*/
  primitive(S(1176), setpagedimen, 0);
  primitive(S(1177), setpagedimen, 1);
  primitive(S(1178), setpagedimen, 2);
  primitive(S(1179), setpagedimen, 3);
  primitive(S(1180), setpagedimen, 4);
  primitive(S(1181), setpagedimen, 5);
  primitive(S(1182), setpagedimen, 6);
  primitive(S(1183), setpagedimen, 7);   /*:983*/
  /*1052:*/
  primitive(S(1184), stop, 0);
  primitive(S(1185), stop, 1);   /*:1052*/
  /*1058:*/
  primitive(S(1186), hskip, skipcode);
  primitive(S(1187), hskip, filcode);
  primitive(S(1188), hskip, fillcode);
  primitive(S(1189), hskip, sscode);
  primitive(S(1190), hskip, filnegcode);
  primitive(S(1191), vskip, skipcode);
  primitive(S(1192), vskip, filcode);
  primitive(S(1193), vskip, fillcode);
  primitive(S(1194), vskip, sscode);
  primitive(S(1195), vskip, filnegcode);
  primitive(S(389), mskip, mskipcode);
  primitive(S(391), kern, explicit);
  primitive(S(393), mkern, muglue);   /*:1058*/
  /*1071:*/
  primitive(S(1196), hmove, 1);
  primitive(S(1197), hmove, 0);
  primitive(S(1198), vmove, 1);
  primitive(S(1199), vmove, 0);
  primitive(S(464), makebox, boxcode);
  primitive(S(1200), makebox, copycode);
  primitive(S(1201), makebox, lastboxcode);
  primitive(S(798), makebox, vsplitcode);
  primitive(S(1202), makebox, vtopcode);
  primitive(S(800), makebox, vtopcode + vmode);
  primitive(S(1203), makebox, vtopcode + hmode);
  primitive(S(1204), leadership, aleaders - 1);
  primitive(S(1205), leadership, aleaders);
  primitive(S(1206), leadership, cleaders);
  primitive(S(1207), leadership, xleaders);   /*:1071*/
  /*1088:*/
  primitive(S(1208), startpar, 1);
  primitive(S(1209), startpar, 0);   /*:1088*/
  /*1107:*/
  primitive(S(1210), removeitem, penaltynode);
  primitive(S(1211), removeitem, kernnode);
  primitive(S(1212), removeitem, gluenode);
  primitive(S(1213), unhbox, boxcode);
  primitive(S(1214), unhbox, copycode);
  primitive(S(1215), unvbox, boxcode);
  primitive(S(1216), unvbox, copycode);   /*:1107*/
  /*1114:*/
  primitive('-', discretionary, 1);
  primitive(S(400), discretionary, 0);   /*:1114*/
  /*1141:*/
  primitive(S(1217), eqno, 0);
  primitive(S(1218), eqno, 1);   /*:1141*/
  /*1156:*/
  ...
  primitive(S(1219), limitswitch, normal);
  ...
  /*1178:*/
  primitive(S(1220), above, abovecode);
  primitive(S(1221), above, overcode);
  primitive(S(1222), above, atopcode);
  primitive(S(1223), above, delimitedcode);
  primitive(S(1224), above, delimitedcode + overcode);
  primitive(S(1225), above, delimitedcode + atopcode);   /*:1178*/
  /*1188:*/
  ...
  /*1208:*/
  primitive(S(960), prefix, 1);
  primitive(S(962), prefix, 2);
  primitive(S(1226), prefix, 4);
  primitive(S(1227), def, 0);
  primitive(S(1228), def, 1);
  primitive(S(1229), def, 2);
  primitive(S(1230), def, 3);
  /*:1208*/
  /*1219:*/
  primitive(S(1231), let, normal);
  primitive(S(1232), let, normal + 1);   /*:1219*/
  /*1222:*/
  primitive(S(1233), shorthanddef, chardefcode);
  primitive(S(1234), shorthanddef, mathchardefcode);
  primitive(S(1235), shorthanddef, countdefcode);
  primitive(S(1236), shorthanddef, dimendefcode);
  primitive(S(1237), shorthanddef, skipdefcode);
  primitive(S(1238), shorthanddef, muskipdefcode);
  primitive(S(1239), shorthanddef, toksdefcode);   /*:1222*/
  /*1230:*/
  ...
  /*:1230*/
  /*1250:*/
  primitive(S(788), hyphdata, 0);
  primitive(S(775), hyphdata, 1);   /*:1250*/
  /*1254:*/
  primitive(S(1240), assignfontint, 0);
  primitive(S(1241), assignfontint, 1);   /*:1254*/
  /*1262:*/
  ...
  primitive(S(1242), setinteraction, errorstopmode);   /*:1262*/
  /*1272:*/
  primitive(S(1243), instream, 1);
  primitive(S(1244), instream, 0);   /*:1272*/
  /*1277:*/
  primitive(S(1245), message, 0);
  primitive(S(1246), message, 1);   /*:1277*/
  /*1286:*/
  primitive(S(1247), caseshift, lccodebase);
  primitive(S(1248), caseshift, uccodebase);   /*:1286*/
  /*1291:*/
  primitive(S(1249), xray, showcode);
  primitive(S(1250), xray, showboxcode);
  primitive(S(1251), xray, showthecode);
  primitive(S(1252), xray, showlists);   /*:1291*/
  /*1344:*/
  ...
  primitive(S(1253), extension, immediatecode);
