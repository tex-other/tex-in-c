...
label labcontinue,exit;var c:ASCIIcode;s1,s2,s3,s4:integer;
...
while true do begin labcontinue:clearforerrorprompt;begin;
...
showcontext;goto labcontinue;end{:88};{_DEBUG}tobar('D'):begin debughelp;
goto labcontinue;end;{_ENDDEBUG}
...
helpline[0]:=strprotector(299);end;goto labcontinue;end{:89};
...
label exit,labcontinue,done,done1,found;var r:pointer;p:pointer;q:pointer;
...
m:=0;end;{392:}labcontinue:gettoken;if curtok=fooinfo(r)^then{394:}
begin r:=foolink(r)^;
...
end else goto labcontinue;end{:394};{397:}if s<>r then if s=0 then{398:}
begin begin if interaction=errorstopmode then;printnl(258);
...
else begin r:=foolink(v)^;goto labcontinue;end;
...
fooinfo(r)^>=matchtoken then goto labcontinue;storenewtoken(curtok);
end{:393};m:=m+1;if fooinfo(r)^>endmatchtoken then goto labcontinue;
if fooinfo(r)^<matchtoken then goto labcontinue;found:if s<>0 then{400:}
...
while d<k+bc-fmemptr do begin qw:=foocharinfo ( f,d
)^;if chartag(qw)<>listtag then goto notfound;
...
foocharinfo ( f,c+minquarterword )^)then
...
curh:=curh+foocharwidth ( f,foocharinfo ( f,
c )^)^;p:=foolink(p)^;
...
hd:=fooheightdepth(i)^;
x:=x+foocharwidth ( f,i )^;
s:=foocharheight ( f,hd )^;if s>h then h:=s;
s:=foochardepth ( f,hd )^;if s>d then d:=s;
p:=foolink(p)^;end{:654};
...
begin q:=foocharinfo ( f,c )^;
hd:=fooheightdepth(q)^;b:=newnullbox;
foowidth(b)^:=foocharwidth ( f,q )^+
foocharitalic ( f,q )^;
...
begin q:=foocharinfo ( f,c )^;
hd:=fooheightdepth(q)^;
heightplusdepth:=foocharheight ( f,hd )^+
foochardepth ( f,hd )^;end;{:712}
function vardelimiter(d:pointer;s:smallnumber;v:scaled):pointer;
label found,labcontinue;var b:pointer;f,g:internalfontnumber;
...
q:=foocharinfo ( f,c )^;
foowidth(b)^:=foocharwidth ( f,q )^+
foocharitalic ( f,q )^;c:=fooextbot(r)^;
...
v:=foocharwidth ( f,foocharinfo ( f,
foocharacter(p)^)^)^;
...
<=fontec[curf])then curi:=foocharinfo ( curf,curc
...
stopflag then s:=foocharkern ( curf,curi )^;
...
y:=foorembyte(i)^;i:=foocharinfo ( f,y )^;
if not charexists(i)then goto done;
if foocharwidth ( f,i )^>w then goto done;
c:=y;end;done:{:740};
...
if charexists(curi)then begin delta:=foocharitalic ( curf,
curi )^;p:=newcharacter(curf,curc-minquarterword);
...
procedure initalign;label done,done1,done2,labcontinue;
...
foolink(p)^:=0;while true do begin labcontinue:getpreambletoken;
...
helpline[2]:=strprotector(903);helpline[1]:=strprotector(904);
helpline[0]:=strprotector(907);end;error;goto labcontinue;end;
...
label exit,done,done1,labcontinue,deactivate;var r:pointer;prevr:pointer;
...
breakwidth[1]:=breakwidth[1]-foocharwidth ( f,foocharinfo
( f,foocharacter(v)^)^)^;
end else case footype(v)^of ligaturenode:begin f:=foofont(ligchar(v))^;
breakwidth[1]:=breakwidth[1]-foocharwidth ( f,foocharinfo
( f,foocharacter(ligchar(v))^)^
)^;end;
...
else begin prevr:=r;if b>threshold then goto labcontinue;
...
if noderstaysactive then goto labcontinue;deactivate:{860:}
...
label labcontinue,done;var p:pointer;t:pointer;q:fourquarters;
...
hchar:=nonchar;currh:=nonchar;goto labcontinue;
...
minquarterword then goto done;goto labcontinue;end{:911};
w:=foocharkern ( hf,q )^;goto done;end;
if fooskipbyte(q)^>=stopflag then if currh=nonchar then goto done else
begin currh:=nonchar;goto labcontinue;end;
...
ligaturepresent:=true;popligstack;goto labcontinue;end{:910};
...
label done,done1,done2,done3,done4,done5,labcontinue;var{862:}
...
actwidth:=actwidth+foocharwidth ( f,foocharinfo
( f,foocharacter(curp)^)^)^;
...
labcontinue else begin q:=fooligptr(s)^;c:=foocharacter(q)^-minquarterword;
hf:=foofont(q)^;
end else if(footype(s)^=kernnode)and(foosubtype(s)^=normal)then goto
labcontinue else if footype(s)^=whatsitnode then begin{1363:}
advpast(s){:1363};goto labcontinue;end else goto done1;
if foolccode(c)^<>0 then if(foolccode(c)^=c)or(uchyph>0)then goto done2
else goto done1;labcontinue:prevs:=s;s:=foolink(prevs)^;end;
...
actwidth:=actwidth+foocharwidth ( f,foocharinfo
( f,foocharacter(ligchar(curp))^)^
)^;end;discnode:{869:}begin s:=fooprebreak(curp)^;
discwidth:=0;
if s=0 then trybreak(exhyphenpenalty,hyphenated)else begin repeat{870:}
if ischarnode(s)then begin f:=foofont(s)^;
discwidth:=discwidth+foocharwidth ( f,foocharinfo
( f,foocharacter(s)^)^)^;
end else case footype(s)^of ligaturenode:begin f:=foofont(ligchar(s))^;
discwidth:=discwidth+foocharwidth ( f,foocharinfo
( f,foocharacter(ligchar(s))^)^
...
label exit,done,done1,labcontinue,contribute,updateheights;var p:pointer;
q,r:pointer;b,c:integer;pi:integer;n:minquarterword..255;
delta,h,w:scaled;
begin if(foolink(contribhead)^=0)or outputactive then goto exit;
repeat labcontinue:p:=foolink(contribhead)^;{996:}
...
foolink(contribhead)^:=q;goto labcontinue;end{:1001}else{1002:}
...
f:=foofont(p)^;
tailappend(newkern(foocharitalic ( f,foocharinfo
( f,foocharacter(p)^)^)^));
...
a:=foocharwidth ( f,foocharinfo ( f,
foocharacter(p)^)^)^;doassignments;{1124:}
q:=0;f:=curfont;
...
d:=foocharwidth ( f,foocharinfo ( f,
foocharacter(p)^)^)^;goto found;end;
