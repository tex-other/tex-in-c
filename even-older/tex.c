 ...
 * Note: Evaluating FOR loop limit before initial value [315] */
  ...
  text(frozendontexpand) = S(513);   /*:258*/
  /*552:*/
  fontptr = nullfont;
  fmemptr = 7;
  ...
  trieptr = 0;   /*:951*/
  /*1216:*/
  text(frozenprotection) = S(1189);   /*:1216*/
  /*1301:*/
  formatident = S(1256);   /*:1301*/
  /*1369:*/
  text(endwrite) = S(1295);
...
Static void shortdisplay(long p)
{
  ...
  while (p > memmin) {
	...
	if (font(p) != fontinshortdisplay) {
	  if ((font(p) < fontbase) | (font(p) > fontmax))
	    printchar('*');
	  else  /*267:*/
		/*:267*/
...
223:*/
  ...
  if (n < localbase) {   /*229:*/
	...
	printspec(equiv(n), S(268));
      else
	printspec(equiv(n), S(350));
      return;
    }
    if (n < muskipbase) {
      printesc(S(407));
...
_Lfound:
  if (f != nullfont) {   /*710:*/
    if (chartag(q) == exttag) {   /*713:*/
      b = newnullbox();
      type(b) = vlistnode;
      r = fontinfo[extenbase[f - fontbase] + rembyte(q)].qqqq;   /*714:*/
      c = extrep(r);
...
/* p2c: tex1.p: Warning: Symbol 'CHARTAG' is not defined [221] */
      a = ligkernbase[curf - fontbase] + rembyte(curi);
      curi = fontinfo[a].qqqq;
      if (skipbyte(curi) > stopflag) {
	a = ligkernbase[curf - fontbase] + opbyte(curi) * 256 +
	    rembyte(curi) - kernbaseoffset + 32768L;
	curi = fontinfo[a].qqqq;
      }
      while (true) {
	if (nextchar(curi) - minquarterword == skewchar[curf - fontbase]) {
...
/* p2c: tex1.p: Warning: Symbol 'CHARTAG' is not defined [221] */
    ...
    k = ligkernbase[hf - fontbase] + rembyte(q);
    q = fontinfo[k].qqqq;
    if (skipbyte(q) > stopflag) {
      k = ligkernbase[hf - fontbase] + opbyte(q) * 256 +
	  rembyte(q) - kernbaseoffset + 32768L;
      q = fontinfo[k].qqqq;
    }
  }
  if (currh < nonchar)
...
Static void appspace(void)
{
  ...
  if (spacefactor >= 2000 && xspaceskip != zeroglue)
	...
	stretch(mainp) = fontinfo[maink + 1].sc;
	shrink(mainp) = fontinfo[maink + 2].sc;
...
Static void aftermath(void)
{
  ...
  danger = false;   /*1195:*/
  if ((fontparams[famfnt(textsize + 2) - fontbase] < totalmathsyparams) |
      (fontparams[famfnt(scriptsize + 2) - fontbase] < totalmathsyparams) |
      (fontparams[famfnt(scriptscriptsize + 2) - fontbase] <
       totalmathsyparams)) {
    printnl(258);
    print(S(1157));
    helpptr = 3;
    ...
    flushmath();
    danger = true;
  } else if ((fontparams[famfnt(textsize + 3) - fontbase] <
	      totalmathexparams) |
	     (fontparams[famfnt(scriptsize + 3) - fontbase] <
	      totalmathexparams) |
	     (fontparams[famfnt(scriptscriptsize + 3) - fontbase] <
	      totalmathexparams)) {
    printnl(258);
    print(S(1161));
    helpptr = 3;
...
/* p2c: tex1.p: Warning: Symbol 'CHARTAG' is not defined [221] */
  ...
  maink = ligkernbase[mainf - fontbase] + rembyte(maini);
  mainj = fontinfo[maink].qqqq;
  if (skipbyte(mainj) <= stopflag)
    goto _Lmainligloop2;
  maink = ligkernbase[mainf - fontbase] + opbyte(mainj) * 256 +
	  rembyte(mainj) - kernbaseoffset + 32768L;
_Lmainligloop1:
  mainj = fontinfo[maink].qqqq;
_Lmainligloop2:
...
Static void initprim(void)
{
  ...
  text(frozenendtemplate) = S(900);
  text(frozenendv) = S(900);
