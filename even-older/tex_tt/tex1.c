...
Static long mode, head, tail, prevgraf, aux, prevdepth, spacefactor, clang,
	    incompleatnoad, modeline, topmark, firstmark, botmark,
	    splitfirstmark, splitbotmark, nonchar, stopflag, kernflag,
	    defaultrulethickness, bigopspacing1, bigopspacing2, bigopspacing3,
	    bigopspacing4, bigopspacing5;
Static scaled activeheight[6];
Static triepointer trieroot;
Static scaled actwidth, curheight, pagegoal, pagetotal, pageshrink, pagedepth,
	      texremainder, maxv, maxh, ruleht, ruledp, rulewd;
Static FILE *curfile;
Static quarterword state, iindex, tokentype;
Static pointer contribtail;
Static halfword start, loc, limit, name, paramstart, parshapeptr,
		outputroutine, everypar, everymath, everydisplay, everyhbox,
		everyvbox, everyjob, everycr, errhelp, curfont;
Static long lineskip, baselineskip, parskip, abovedisplayskip,
	    belowdisplayskip, abovedisplayshortskip, belowdisplayshortskip,
	    leftskip, rightskip, topskip, splittopskip, tabskip, spaceskip,
	    xspaceskip, parfillskip, thinmuskip, medmuskip, thickmuskip,
	    pretolerance, tolerance, linepenalty, hyphenpenalty,
	    exhyphenpenalty, clubpenalty, widowpenalty, displaywidowpenalty,
	    brokenpenalty, binoppenalty, relpenalty, predisplaypenalty,
	    postdisplaypenalty, interlinepenalty, doublehyphendemerits,
	    finalhyphendemerits, adjdemerits, mag, delimiterfactor, looseness,
	    time, day, month, year, showboxbreadth, showboxdepth, hbadness,
	    vbadness, pausing, tracingonline, tracingmacros, tracingstats,
	    tracingparagraphs, tracingpages, tracingoutput, tracinglostchars,
	    tracingcommands, tracingrestores, uchyph, outputpenalty,
	    maxdeadcycles, hangafter, floatingpenalty, globaldefs, curfam,
	    escapechar, defaulthyphenchar, defaultskewchar, endlinechar,
	    newlinechar, language, lefthyphenmin, righthyphenmin,
	    holdinginserts, errorcontextlines, parindent, mathsurround,
	    lineskiplimit, hsize, vsize, maxdepth, splitmaxdepth, boxmaxdepth,
	    hfuzz, vfuzz, delimitershortfall, nulldelimiterspace, scriptspace,
	    predisplaysize, displaywidth, displayindent, overfullrule,
	    hangindent, hoffset, voffset, emergencystretch;
...
Static long strprotector(long x);
 ...
 * Note: Evaluating FOR loop limit before initial value [315] */
  ...
  *footext(frozendontexpand) = strprotector(513);   /*:258*/
  /*552:*/
  fontptr = nullfont;
  fmemptr = 7;
  fontname[nullfont - fontbase] = strprotector(803);
  fontarea[nullfont - fontbase] = strprotector(266);
  hyphenchar[nullfont - fontbase] = tobar("-");
  ...
  trieptr = 0;   /*:951*/
  /*1216:*/
  *footext(frozenprotection) = strprotector(1189);   /*:1216*/
  /*1301:*/
  formatident = strprotector(1256);   /*:1301*/
  /*1369:*/
  *footext(endwrite) = strprotector(1295);
...
Static void printthedigs(eightbits k)
{
  while (k > 0) {
    k--;
    if (dig[k] < 10)
      printchar(tobar("0") + dig[k]);
    else
      printchar(tobar("A") + dig[k] - 10);
  }
}
...
Static void printcs(long p)
{
  if (p < hashbase) {
    if (p >= singlebase) {
      if (p == nullcs) {
	printesc(strprotector(515));
	printesc(strprotector(516));
	return;
      }
...
{
  if (s == 0) {
    printesc(strprotector(423));
    return;
  }
  if (s == scriptsize)
    printesc(strprotector(424));
  else
    printesc(strprotector(425));
}  /*:699*/
...
_Llabcontinue:
	...
	if (deletionsallowed) {   /*88:*/
	  ...
	  alignstate = 1000000L;
	  OKtointerrupt = false;
	  if ((last > first + 1) & (buffer[first + 1] >= tobar("0")) &
	      (buffer[first + 1] <= tobar("9")))
	    c = c * 10 + buffer[first + 1] - tobar("0") * 11;
	  else
	    c -= tobar("0");
	  while (c > 0) {
	    gettoken();
	  ...
	  OKtointerrupt = true;
	  helpptr = 2;
	  helpline[1] = strprotector(293);
	  helpline[0] = strprotector(294);
	  showcontext();
	  goto _Llabcontinue;
	}
	/*:88*/
	...
	/*_ENDDEBUG*/
      ...
      case tobar("E"):
	if (baseptr > 0) {
	  printnl(strprotector(279));
	  slowprint(inputstack[baseptr].namefield);
	  print(strprotector(280));
	  printint(line);
	...
	if (useerrhelp) {
	  ...
	  if (helpptr == 0) {
	    helpptr = 2;
	    helpline[1] = strprotector(295);
	    helpline[0] = strprotector(296);
	  }
	  do {
	    helpptr--;
	...
	helpptr = 4;
	helpline[3] = strprotector(297);
	helpline[2] = strprotector(296);
	helpline[1] = strprotector(298);
	helpline[0] = strprotector(299);
	goto _Llabcontinue;
	break;
	/*:89*/
	...
	/*:87*/
      ...
      case tobar("S"):   /*86:*/
	errorcount = 0;
	interaction = batchmode + c - tobar("Q");
	print(strprotector(287));
	switch (c) {

	case tobar("Q"):
	  printesc(strprotector(288));
	  selector--;
	  break;

	case tobar("R"):
	  printesc(strprotector(289));
	  break;

	case tobar("S"):
	  printesc(strprotector(290));
	  break;
	}
	print(strprotector(291));
	println();
	fflush(termout);
/* p2c: tex1.p: Warning: Symbol 'FFLUSH' is not defined [221] */
	...
	break;
      ...
      print(strprotector(281));
      printnl(strprotector(282));
      printnl(strprotector(283));
      if (baseptr > 0)
	print(strprotector(284));
      if (deletionsallowed)
	printnl(strprotector(285));
      printnl(strprotector(286));   /*:85*/
      /*:84*/
    }
  }
  errorcount++;
  if (errorcount == 100) {
    printnl(strprotector(277));
...
Static void overflow(strnumber s, long n)
{
  normalizeselector();
  printnl(258);
  print(strprotector(302));
  print(s);
  printchar(tobar("="));
  printint(n);
  printchar(tobar("]"));
  helpptr = 2;
  helpline[1] = strprotector(303);
  helpline[0] = strprotector(304);
  if (interaction == errorstopmode)
    interaction = scrollmode;
  if (logopened)   /*_DEBUG*/
    error();
...
Static void confusion(strnumber s)
{
  ...
  if (history < errormessageissued) {
    printnl(258);
    print(strprotector(305));
    print(s);
    printchar(tobar(")"));
    helpptr = 1;
    helpline[0] = strprotector(306);
  } else {
    printnl(258);
    print(strprotector(307));
    helpptr = 2;
    helpline[1] = strprotector(308);
    helpline[0] = strprotector(309);
  }
  if (interaction == errorstopmode)
    interaction = scrollmode;
  if (logopened)   /*_DEBUG*/
  ...
  if (P_eof(*f))
	...
	if (maxbufstack == bufsize) {   /*35:*/
	  if (formatident == 0) {
	    ...
	    curinput.limitfield = last - 1;
	    overflow(strprotector(272), bufsize);
	  }  /*:35*/
	}
      }
...
/* p2c: tex1.p: Warning: Char constant with more than one character [154] */
    ...
    loc = first;
    while ((loc < last) & (buffer[loc] == tobar(" ")))
      loc++;
    if (loc < last) {
      Result = true;
      goto _Lexit;
...
Static strnumber makestring(void)
{
  if (strptr == maxstrings)
    overflow(strprotector(273), maxstrings - initstrptr);
  strptr++;
...
Static void printromanint(long n)
{
  ...
  while (true) {
    ...
    if (n <= 0)
      goto _Lexit;
    k = j + 2;
    u = v / (strpool[k - 1] - tobar("0"));
    if (strpool[k - 1] == tobar("2")) {
      k += 2;
      u /= strpool[k - 1] - tobar("0");
    }
    if (n + u >= v) {
      printchar(strpool[k]);
      n += u;
...
Static void pauseforinstructions(void)
{
  ...
  if (selector == logonly || selector == noprint)
    selector++;
  printnl(258);
  print(strprotector(310));
  helpptr = 3;
  helpline[2] = strprotector(311);
  helpline[1] = strprotector(312);
  helpline[0] = strprotector(313);
...
Static void runaway(void)
{
  ...
  if (scannerstatus <= skipping)
    return;
  printnl(strprotector(579));
  switch (scannerstatus) {

  case defining:
    print(strprotector(580));
    p = defref;
    break;

  case matching:
    print(strprotector(581));
    p = temphead;
    break;

  case aligning:
    print(strprotector(582));
    p = holdhead;
    break;

  case absorbing:
    print(strprotector(583));
    p = defref;
    break;
  }
  printchar(tobar("?"));
...
Static halfword getavail(void)
{
  ...
  if (p != 0)
    ...
    if (himemmin <= lomemmax) {
      runaway();
      overflow(strprotector(314), memmax - memmin + 1);
    }
  }
  *foolink(p) = 0;
  /*_STAT*/
...
Static void searchmem(halfword p)
{
  ...
  for (q = memmin; q <= FORLIM; q++) {
    if (*foolink(q) == p) {
      printnl(strprotector(320));
      printint(q);
      printchar(tobar(")"));
    }
    if (*fooinfo(q) == p) {
      printnl(strprotector(321));
      printint(q);
      printchar(tobar(")"));
    }
  }
...
Static void printspec(long p, strnumber s)
{
  ...
  if (s != 0)
    print(s);
  if (*foostretch(p) != 0) {
    print(strprotector(270));
    printglue(*foostretch(p), *foostretchorder(p), s);
  }
  if (*fooshrink(p) != 0) {
    print(strprotector(326));
...
Static void printstyle(long c)
{
  ...
  case 0:
    printesc(strprotector(861));
    break;

  case 1:
    printesc(strprotector(862));
    break;

  case 2:
    printesc(strprotector(863));
    break;

  case 3:
    printesc(strprotector(864));
    break;

  default:
    print(strprotector(865));
    break;
  }
}
...
Static void printskipparam(long n)
{
  ...
  case lineskipcode:
    printesc(strprotector(388));
    break;

  case baselineskipcode:
    printesc(strprotector(389));
    break;

  case parskipcode:
    printesc(strprotector(390));
    break;

  case abovedisplayskipcode:
    printesc(strprotector(391));
    break;

  case belowdisplayskipcode:
    printesc(strprotector(392));
    break;

  case abovedisplayshortskipcode:
    printesc(strprotector(393));
    break;

  case belowdisplayshortskipcode:
    printesc(strprotector(394));
    break;

  case leftskipcode:
    printesc(strprotector(395));
    break;

  case rightskipcode:
    printesc(strprotector(396));
    break;

  case topskipcode:
    printesc(strprotector(397));
    break;

  case splittopskipcode:
    printesc(strprotector(398));
    break;

  case tabskipcode:
    printesc(strprotector(399));
    break;

  case spaceskipcode:
    printesc(strprotector(400));
    break;

  case xspaceskipcode:
    printesc(strprotector(401));
    break;

  case parfillskipcode:
    printesc(strprotector(402));
    break;

  case thinmuskipcode:
    printesc(strprotector(403));
    break;

  case medmuskipcode:
    printesc(strprotector(404));
    break;

  case thickmuskipcode:
    printesc(strprotector(405));
    break;

  default:
    print(strprotector(406));
    break;
  }
}
...
/* p2c: tex1.p: Warning: Symbol 'CURLENGTH' is not defined [221] */
  ...
  while (p > memmin) {
    ...
    printcurrentstring();
    if (p > memend) {
      print(strprotector(328));
      goto _Lexit;
    }
    n++;
    if (n > breadthmax) {
      print(strprotector(329));
      goto _Lexit;
    }  /*183:*/
    if (ischarnode(p))
      printfontandchar(p);
	...
	if (*footype(p) == unsetnode) {   /*185:*/
	  ...
	  if ((g != 0.0) & (*foogluesign(p) != normal)) {   /*:186*/
	    print(strprotector(338));
	    if (*foogluesign(p) == shrinking)
	      print(strprotector(339));
	    if (labs(mem[p + glueoffset - memmin].int_) < 1048576L)
	      print(strprotector(340));
	    else {
	      if (fabs(g) > 20000.0) {
		if (g > 0.0)
		  printchar(tobar(">"));
		else
		  print(strprotector(341));
...
/* p2c: tex1.p: Warning: Symbol 'NODELISTDISPLAY' is not defined [221] */
	...
	/*:187*/
      ...
      case insnode:   /*188:*/
	printesc(strprotector(343));
	printint(*foosubtype(p) - minquarterword);
	print(strprotector(344));
	printscaled(*fooheight(p));
	print(strprotector(345));
	printspec(*foosplittopptr(p), 0);
	printchar(tobar(","));
	printscaled(*foodepth(p));
	print(strprotector(346));
...
/* p2c: tex1.p: Warning: Symbol 'NODELISTDISPLAY' is not defined [221] */
	...
	case opennode:
	  printwritewhatsit(strprotector(1284), p);
	  printchar(tobar("="));
	  printfilename(*fooopenname(p), *fooopenarea(p), *fooopenext(p));
	  break;

	case writenode:
	  printwritewhatsit(strprotector(603), p);
	  printmark(*foowritetokens(p));
	  break;

	case closenode:
	  printwritewhatsit(strprotector(1285), p);
	  break;

	case specialnode:
	  printesc(strprotector(1286));
	  printmark(*foowritetokens(p));
	  break;

	case languagenode:
	  printesc(strprotector(1288));
	  printint(*foowhatlang(p));
	  print(strprotector(1291));
...
/* p2c: tex1.p: Warning: Symbol 'NODELISTDISPLAY' is not defined [221] */
	  ...
	  if (*foosubtype(p) != normal) {
	    ...
	    if (*foosubtype(p) < condmathglue)
	      ...
	      if (*foosubtype(p) == condmathglue)
		printesc(strprotector(348));
	      else
		printesc(strprotector(349));
	    }
	    printchar(tobar(")"));
	  }
	  if (*foosubtype(p) != condmathglue) {
	    ...
	    if (*foosubtype(p) < condmathglue)
	      ...
	      printspec(*fooglueptr(p), strprotector(350));
	  }
	}
	break;
	...
	if (*foosubtype(p) != muglue) {
	  printesc(strprotector(352));
	  if (*foosubtype(p) != normal)
	    printchar(tobar(" "));
	  printscaled(*foowidth(p));
	  if (*foosubtype(p) == acckern)
	    print(strprotector(353));
	} else {   /*:191*/
	  printesc(strprotector(354));
	  printscaled(*foowidth(p));
	  print(strprotector(350));
	}
	break;

      case mathnode:   /*192:*/
	printesc(strprotector(355));
	if (*foosubtype(p) == before)
	  print(strprotector(356));
	else
	  print(strprotector(357));
	if (*foowidth(p) != 0) {
	  print(strprotector(358));
	  printscaled(*foowidth(p));
	}
	break;
	/*:192*/
...
/* p2c: tex1.p: Warning: Symbol 'LIGCHAR' is not defined [221] */
	...
	printesc(strprotector(360));
	printint(*foopenalty(p));
	break;
	/*:194*/

      case discnode:   /*195:*/
	printesc(strprotector(361));
	if (*fooreplacecount(p) > 0) {
	  print(strprotector(362));
...
/* p2c: tex1.p: Warning: Symbol 'FLUSHCHAR' is not defined [221] */
	...
	/*:195*/
      ...
      case marknode:   /*196:*/
	printesc(strprotector(363));
	printmark(*foomarkptr(p));
	break;
	/*:196*/

      case adjustnode:   /*197:*/
	printesc(strprotector(364));
...
/* p2c: tex1.p: Warning: Symbol 'FLUSHCHAR' is not defined [221] */
	...
	case ordnoad:
	  printesc(strprotector(866));
	  break;

	case opnoad:
	  printesc(strprotector(867));
	  break;

	case binnoad:
	  printesc(strprotector(868));
	  break;

	case relnoad:
	  printesc(strprotector(869));
	  break;

	case opennoad:
	  printesc(strprotector(870));
	  break;

	case closenoad:
	  printesc(strprotector(871));
	  break;

	case punctnoad:
	  printesc(strprotector(872));
	  break;

	case innernoad:
	  printesc(strprotector(873));
	  break;

	case overnoad:
	  printesc(strprotector(874));
	  break;

	case undernoad:
	  printesc(strprotector(875));
	  break;

	case vcenternoad:
	  printesc(strprotector(550));
	  break;

	case radicalnoad:
	  printesc(strprotector(544));
	  printdelimiter(*fooleftdelimiter(p));
	  break;

	case accentnoad:
	  printesc(strprotector(519));
	  printfamandchar(*fooaccentchr(p));
	  break;

	case leftnoad:
	  printesc(strprotector(876));
	  printdelimiter(*foonucleus(p));
	  break;

	case rightnoad:
	  printesc(strprotector(877));
	  printdelimiter(*foonucleus(p));
	  break;
	}
	if (*foosubtype(p) != normal) {
	  if (*foosubtype(p) == limits)
	    printesc(strprotector(878));
	  else
	    printesc(strprotector(879));
	}
	if (*footype(p) < leftnoad)
	  printsubsidiarydata(*foonucleus(p), tobar("."));
	printsubsidiarydata(*foosupscr(p), tobar("^"));
	printsubsidiarydata(*foosubscr(p), tobar("_"));
	break;
	/*:696*/

      case fractionnoad:   /*697:*/
	printesc(strprotector(880));
	if (*foothickness(p) == defaultcode)
	  print(strprotector(881));
	else
	...
	if ((*foosmallfam(*foorightdelimiter(p)) != 0) |
	  ...
	  print(strprotector(883));
	  printdelimiter(*foorightdelimiter(p));
	}
	printsubsidiarydata(*foonumerator(p), tobar("\\"));
	printsubsidiarydata(*foodenominator(p), tobar("/"));
	break;
	/*:697*/
	/*:690*/

      default:
	print(strprotector(330));
	break;
      }
    }
    p = *foolink(p);
...
Static void printmode(long m)
{
  if (m > 0) {
    ...
    case 0:
      print(strprotector(367));
      break;

    case 1:
      print(strprotector(368));
      break;

    case 2:
      print(strprotector(369));
      break;
    }
  } else if (m == 0)
    print(strprotector(370));
  else {
    switch (-m / (maxcommand + 1)) {

    case 0:
      print(strprotector(371));
      break;

    case 1:
      print(strprotector(372));
      break;

    case 2:
      print(strprotector(355));
      break;
    }
  }
  print(strprotector(373));
}
...
Static void pushnest(void)
{
  if (nestptr > maxneststack) {
    ...
    if (nestptr == nestsize)
      overflow(strprotector(374), nestsize);
  }
...
Static void showactivities(void)
{
  ...
  for (TEMP = nestptr; TEMP >= 0; TEMP--) {
    ...
    m = nest[p].modefield;
    a = nest[p].auxfield;
    printnl(strprotector(375));
    printmode(m);
    print(strprotector(376));
    printint(labs(nest[p].mlfield));
    if (m == hmode) {
      if (nest[p].pgfield != 8585216L) {
	print(strprotector(377));
/* p2c: tex1.p: Note: Using % for possibly-negative arguments [317] */
	printint(nest[p].pgfield % 65536L);
	print(strprotector(378));
    ...
    if (nest[p].mlfield < 0)
      print(strprotector(379));
    if (p == 0) {  /*986:*/
      if (pagehead != pagetail) {   /*:986*/
	printnl(strprotector(979));
	if (outputactive)
	  print(strprotector(980));
	showbox(*foolink(pagehead));
	if (pagecontents > empty) {
	  printnl(strprotector(981));
	  printtotals();
	  printnl(strprotector(982));
	  ...
	  while (r != pageinshead) {
		...
		if ((*footype(q) == insnode) & (*foosubtype(q) == *foosubtype(r)))
	      ...
	      print(strprotector(984));
	      printint(t);
	      print(strprotector(985));
	    }
	    r = *foolink(r);
	  }
	}
	...
	printnl(strprotector(380));
    }
    showbox(*foolink(nest[p].headfield));   /*219:*/
    switch (abs(m) / (maxcommand + 1)) {   /*:219*/

    case 0:
      printnl(strprotector(381));
      if (a.sc <= ignoredepth)
	print(strprotector(382));
      else
	printscaled(a.sc);
      if (nest[p].pgfield != 0) {
	print(strprotector(383));
	printint(nest[p].pgfield);
	print(strprotector(384));
	if (nest[p].pgfield != 1)
	  printchar(tobar("s"));
      }
      break;
    ...
    case 1:
      printnl(strprotector(385));
      printint(a.hh.UU.lh);
      if (m > 0) {
	if (a.hh.rh > 0) {
	  print(strprotector(386));
	  printint(a.hh.rh);
	}
      }
      break;
...
Static void printparam(long n)
{
  ...
  case pretolerancecode:
    printesc(strprotector(431));
    break;

  case tolerancecode:
    printesc(strprotector(432));
    break;

  case linepenaltycode:
    printesc(strprotector(433));
    break;

  case hyphenpenaltycode:
    printesc(strprotector(434));
    break;

  case exhyphenpenaltycode:
    printesc(strprotector(435));
    break;

  case clubpenaltycode:
    printesc(strprotector(436));
    break;

  case widowpenaltycode:
    printesc(strprotector(437));
    break;

  case displaywidowpenaltycode:
    printesc(strprotector(438));
    break;

  case brokenpenaltycode:
    printesc(strprotector(439));
    break;

  case binoppenaltycode:
    printesc(strprotector(440));
    break;

  case relpenaltycode:
    printesc(strprotector(441));
    break;

  case predisplaypenaltycode:
    printesc(strprotector(442));
    break;

  case postdisplaypenaltycode:
    printesc(strprotector(443));
    break;

  case interlinepenaltycode:
    printesc(strprotector(444));
    break;

  case doublehyphendemeritscode:
    printesc(strprotector(445));
    break;

  case finalhyphendemeritscode:
    printesc(strprotector(446));
    break;

  case adjdemeritscode:
    printesc(strprotector(447));
    break;

  case magcode:
    printesc(strprotector(448));
    break;

  case delimiterfactorcode:
    printesc(strprotector(449));
    break;

  case loosenesscode:
    printesc(strprotector(450));
    break;

  case timecode:
    printesc(strprotector(451));
    break;

  case daycode:
    printesc(strprotector(452));
    break;

  case monthcode:
    printesc(strprotector(453));
    break;

  case yearcode:
    printesc(strprotector(454));
    break;

  case showboxbreadthcode:
    printesc(strprotector(455));
    break;

  case showboxdepthcode:
    printesc(strprotector(456));
    break;

  case hbadnesscode:
    printesc(strprotector(457));
    break;

  case vbadnesscode:
    printesc(strprotector(458));
    break;

  case pausingcode:
    printesc(strprotector(459));
    break;

  case tracingonlinecode:
    printesc(strprotector(460));
    break;

  case tracingmacroscode:
    printesc(strprotector(461));
    break;

  case tracingstatscode:
    printesc(strprotector(462));
    break;

  case tracingparagraphscode:
    printesc(strprotector(463));
    break;

  case tracingpagescode:
    printesc(strprotector(464));
    break;

  case tracingoutputcode:
    printesc(strprotector(465));
    break;

  case tracinglostcharscode:
    printesc(strprotector(466));
    break;

  case tracingcommandscode:
    printesc(strprotector(467));
    break;

  case tracingrestorescode:
    printesc(strprotector(468));
    break;

  case uchyphcode:
    printesc(strprotector(469));
    break;

  case outputpenaltycode:
    printesc(strprotector(470));
    break;

  case maxdeadcyclescode:
    printesc(strprotector(471));
    break;

  case hangaftercode:
    printesc(strprotector(472));
    break;

  case floatingpenaltycode:
    printesc(strprotector(473));
    break;

  case globaldefscode:
    printesc(strprotector(474));
    break;
  ...
  case escapecharcode:
    printesc(strprotector(476));
    break;

  case defaulthyphencharcode:
    printesc(strprotector(477));
    break;

  case defaultskewcharcode:
    printesc(strprotector(478));
    break;

  case endlinecharcode:
    printesc(strprotector(479));
    break;

  case newlinecharcode:
    printesc(strprotector(480));
    break;

  case languagecode:
    printesc(strprotector(481));
    break;

  case lefthyphenmincode:
    printesc(strprotector(482));
    break;

  case righthyphenmincode:
    printesc(strprotector(483));
    break;

  case holdinginsertscode:
    printesc(strprotector(484));
    break;

  case errorcontextlinescode:
    printesc(strprotector(485));
    break;

  default:
    print(strprotector(486));
    break;
  }
}
...
Static void printlengthparam(long n)
{
  ...
  case parindentcode:
    printesc(strprotector(489));
    break;

  case mathsurroundcode:
    printesc(strprotector(490));
    break;

  case lineskiplimitcode:
    printesc(strprotector(491));
    break;

  case hsizecode:
    printesc(strprotector(492));
    break;

  case vsizecode:
    printesc(strprotector(493));
    break;

  case maxdepthcode:
    printesc(strprotector(494));
    break;

  case splitmaxdepthcode:
    printesc(strprotector(495));
    break;

  case boxmaxdepthcode:
    printesc(strprotector(496));
    break;

  case hfuzzcode:
    printesc(strprotector(497));
    break;

  case vfuzzcode:
    printesc(strprotector(498));
    break;

  case delimitershortfallcode:
    printesc(strprotector(499));
    break;

  case nulldelimiterspacecode:
    printesc(strprotector(500));
    break;

  case scriptspacecode:
    printesc(strprotector(501));
    break;

  case predisplaysizecode:
    printesc(strprotector(502));
    break;

  case displaywidthcode:
    printesc(strprotector(503));
    break;

  case displayindentcode:
    printesc(strprotector(504));
    break;

  case overfullrulecode:
    printesc(strprotector(505));
    break;

  case hangindentcode:
    printesc(strprotector(506));
    break;

  case hoffsetcode:
    printesc(strprotector(507));
    break;

  case voffsetcode:
    printesc(strprotector(508));
    break;

  case emergencystretchcode:
    printesc(strprotector(509));
    break;

  default:
    print(strprotector(510));
    break;
  }
}
...
Static void printcmdchr(quarterword cmd, halfword chrcode)
{
  ...
  case leftbrace:
    chrcmd(strprotector(567));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case rightbrace:
    chrcmd(strprotector(568));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case mathshift:
    chrcmd(strprotector(569));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case macparam:
    chrcmd(strprotector(570));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case supmark:
    chrcmd(strprotector(571));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case submark:
    chrcmd(strprotector(572));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case endv:
    print(strprotector(573));
    break;

  case spacer:
    chrcmd(strprotector(574));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case letter:
    chrcmd(strprotector(575));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;

  case otherchar:
    chrcmd(strprotector(576));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    break;
  ...
  case assignmuglue:   /*:227*/
    if (chrcode < skipbase)
      ...
      printesc(strprotector(407));
      printint(chrcode - skipbase);
    } else {
      printesc(strprotector(408));
  ...
  case assigntoks:   /*:231*/
    if (chrcode >= toksbase) {
      printesc(strprotector(418));
      printint(chrcode - toksbase);
    } else {
      switch (chrcode) {

      case outputroutineloc:
	printesc(strprotector(409));
	break;

      case everyparloc:
	printesc(strprotector(410));
	break;

      case everymathloc:
	printesc(strprotector(411));
	break;

      case everydisplayloc:
	printesc(strprotector(412));
	break;

      case everyhboxloc:
	printesc(strprotector(413));
	break;

      case everyvboxloc:
	printesc(strprotector(414));
	break;

      case everyjobloc:
	printesc(strprotector(415));
	break;

      case everycrloc:
	printesc(strprotector(416));
	break;

      default:
	printesc(strprotector(417));
	break;
      }
    }
    break;
  ...
  case advance:
    printesc(strprotector(520));
    break;

  case afterassignment:
    printesc(strprotector(521));
    break;

  case aftergroup:
    printesc(strprotector(522));
    break;

  case assignfontdimen:
    printesc(strprotector(530));
    break;

  case begingroup:
    printesc(strprotector(523));
    break;

  case breakpenalty:
    printesc(strprotector(542));
    break;

  case charnum:
    printesc(strprotector(524));
    break;
  ...
  case deffont:
    printesc(strprotector(529));
    break;

  case delimnum:
    printesc(strprotector(525));
    break;

  case divide:
    printesc(strprotector(526));
    break;
  ...
  case expandafter:
    printesc(strprotector(528));
    break;

  case halign:
    printesc(strprotector(531));
    break;

  case hrule:
    printesc(strprotector(532));
    break;

  case ignorespaces:
    printesc(strprotector(533));
    break;
  ...
  case mathaccent:
    printesc(strprotector(534));
    break;

  case mathcharnum:
    printesc(strprotector(535));
    break;
  ...
  case multiply:
    printesc(strprotector(537));
    break;

  case noalign:
    printesc(strprotector(538));
    break;

  case noboundary:
    printesc(strprotector(539));
    break;

  case noexpand:
    printesc(strprotector(540));
    break;
  ...
  case readtocs:
    printesc(strprotector(545));
    break;

  case relax:
    printesc(strprotector(546));
    break;

  case setbox:
    printesc(strprotector(547));
    break;

  case setprevgraf:
    printesc(strprotector(543));
    break;

  case setshape:
    printesc(strprotector(419));
    break;

  case the:
    printesc(strprotector(548));
    break;
  ...
  case vrule:
    printesc(strprotector(551));
    break;

  /*:266*/
  /*335:*/
  case parend:
    printesc(strprotector(606));
    break;
  ...
  case input:   /*:377*/
    if (chrcode == 0)
      printesc(strprotector(638));
    else
      printesc(strprotector(639));
    break;
    /*385:*/
  ...
  case topbotmark:   /*:385*/
    ...
    case firstmarkcode:
      printesc(strprotector(641));
      break;

    case botmarkcode:
      printesc(strprotector(642));
      break;

    case splitfirstmarkcode:
      printesc(strprotector(643));
      break;

    case splitbotmarkcode:
      printesc(strprotector(644));
      break;

    default:
      printesc(strprotector(640));
      break;
    }
    break;
    /*412:*/
  ...
  case setaux:
    if (chrcode == vmode)
      printesc(strprotector(678));
    else
      printesc(strprotector(677));
    break;

  case setpageint:
    if (chrcode == 0)
      printesc(strprotector(679));
    else
      printesc(strprotector(680));
    break;

  case setboxdimen:
    if (chrcode == widthoffset)
      printesc(strprotector(681));
    else {
      if (chrcode == heightoffset)
	printesc(strprotector(682));
      else
	printesc(strprotector(683));
    }
    break;
  ...
  case lastitem:   /*:417*/
    ...
    case intval:
      printesc(strprotector(684));
      break;

    case dimenval:
      printesc(strprotector(685));
      break;

    case glueval:
      printesc(strprotector(686));
      break;

    case inputlinenocode:
      printesc(strprotector(687));
      break;

    default:
      printesc(strprotector(688));
      break;
    }
    break;
    /*469:*/
  ...
  case convert:   /*:469*/
    ...
    case numbercode:
      printesc(strprotector(744));
      break;

    case romannumeralcode:
      printesc(strprotector(745));
      break;

    case stringcode:
      printesc(strprotector(746));
      break;

    case meaningcode:
      printesc(strprotector(747));
      break;

    case fontnamecode:
      printesc(strprotector(748));
      break;

    default:
      printesc(strprotector(749));
      break;
    }
    break;
    /*488:*/
  ...
  case iftest:   /*:488*/
    ...
    case ifcatcode:
      printesc(strprotector(765));
      break;

    case ifintcode:
      printesc(strprotector(766));
      break;

    case ifdimcode:
      printesc(strprotector(767));
      break;

    case ifoddcode:
      printesc(strprotector(768));
      break;

    case ifvmodecode:
      printesc(strprotector(769));
      break;

    case ifhmodecode:
      printesc(strprotector(770));
      break;

    case ifmmodecode:
      printesc(strprotector(771));
      break;

    case ifinnercode:
      printesc(strprotector(772));
      break;

    case ifvoidcode:
      printesc(strprotector(773));
      break;

    case ifhboxcode:
      printesc(strprotector(774));
      break;

    case ifvboxcode:
      printesc(strprotector(775));
      break;

    case ifxcode:
      printesc(strprotector(776));
      break;

    case ifeofcode:
      printesc(strprotector(777));
      break;

    case iftruecode:
      printesc(strprotector(778));
      break;

    case iffalsecode:
      printesc(strprotector(779));
      break;

    case ifcasecode:
      printesc(strprotector(780));
      break;

    default:
      printesc(strprotector(764));
      break;
    }
    break;
    /*492:*/
  ...
  case fiorelse:   /*:492*/
    if (chrcode == ficode)
      printesc(strprotector(781));
    else if (chrcode == orcode)
      printesc(strprotector(782));
    else
      printesc(strprotector(783));
    break;
    /*781:*/

  case tabmark:
    if (chrcode == spancode)
      printesc(strprotector(897));
    else {
      chrcmd(strprotector(901));
/* p2c: tex1.p: Warning: Symbol 'CHRCMD' is not defined [221] */
    }
    break;
  ...
  case carret:   /*:781*/
    if (chrcode == crcode)
      printesc(strprotector(898));
    else
      printesc(strprotector(899));
    break;
    /*984:*/
  ...
  case setpagedimen:   /*:984*/
    ...
    case 0:
      printesc(strprotector(969));
      break;

    case 1:
      printesc(strprotector(970));
      break;

    case 2:
      printesc(strprotector(971));
      break;

    case 3:
      printesc(strprotector(972));
      break;

    case 4:
      printesc(strprotector(973));
      break;

    case 5:
      printesc(strprotector(974));
      break;

    case 6:
      printesc(strprotector(975));
      break;

    default:
      printesc(strprotector(976));
      break;
    }
    break;
    /*1053:*/
  ...
  case stop:   /*:1053*/
    if (chrcode == 1)
      printesc(strprotector(1025));
    else
      printesc(strprotector(1024));
    break;
    /*1059:*/
  ...
  case hskip:
    ...
    case skipcode:
      printesc(strprotector(1026));
      break;

    case filcode:
      printesc(strprotector(1027));
      break;

    case fillcode:
      printesc(strprotector(1028));
      break;

    case sscode:
      printesc(strprotector(1029));
      break;

    default:
      printesc(strprotector(1030));
      break;
    }
    break;
  ...
  case vskip:
    ...
    case skipcode:
      printesc(strprotector(1031));
      break;

    case filcode:
      printesc(strprotector(1032));
      break;

    case fillcode:
      printesc(strprotector(1033));
      break;

    case sscode:
      printesc(strprotector(1034));
      break;

    default:
      printesc(strprotector(1035));
      break;
    }
    break;
  ...
  case hmove:
    if (chrcode == 1)
      printesc(strprotector(1053));
    else
      printesc(strprotector(1054));
    break;

  case vmove:
    if (chrcode == 1)
      printesc(strprotector(1055));
    else
      printesc(strprotector(1056));
    break;
  ...
  case makebox:
    ...
    case boxcode:
      printesc(strprotector(420));
      break;

    case copycode:
      printesc(strprotector(1057));
      break;

    case lastboxcode:
      printesc(strprotector(1058));
      break;

    case vsplitcode:
      printesc(strprotector(964));
      break;

    case vtopcode:
      printesc(strprotector(1059));
      break;

    case vtopcode + vmode:
      printesc(strprotector(966));
      break;

    default:
      printesc(strprotector(1060));
      break;
    }
    break;
  ...
  case leadership:   /*:1072*/
    if (chrcode == aleaders)
      printesc(strprotector(1062));
    else if (chrcode == cleaders)
      printesc(strprotector(1063));
    else if (chrcode == xleaders)
      printesc(strprotector(1064));
    else
      printesc(strprotector(1061));
    break;
    /*1089:*/

  case startpar:   /*:1089*/
    if (chrcode == 0)
      printesc(strprotector(1080));
    else
      printesc(strprotector(1079));
    break;
    /*1108:*/

  case removeitem:
    if (chrcode == gluenode)
      printesc(strprotector(1091));
    else if (chrcode == kernnode)
      printesc(strprotector(1090));
    else
      printesc(strprotector(1089));
    break;

  case unhbox:
    if (chrcode == copycode)
      printesc(strprotector(1093));
    else
  ...
  case unvbox:   /*:1108*/
    if (chrcode == copycode)
      printesc(strprotector(1095));
    else
      printesc(strprotector(1094));
    break;
    /*1115:*/
  ...
  case eqno:   /*:1143*/
    if (chrcode == 1)
      printesc(strprotector(1127));
    else
      printesc(strprotector(1126));
    break;
    /*1157:*/
  ...
  case above:   /*:1179*/
    ...
    case overcode:
      printesc(strprotector(1147));
      break;

    case atopcode:
      printesc(strprotector(1148));
      break;

    case delimitedcode:
      printesc(strprotector(1149));
      break;

    case delimitedcode + overcode:
      printesc(strprotector(1150));
      break;

    case delimitedcode + atopcode:
      printesc(strprotector(1151));
      break;

    default:
      printesc(strprotector(1146));
      break;
    }
    break;
    /*1189:*/
  ...
  case prefix:
    if (chrcode == 1)
      printesc(strprotector(1170));
    else if (chrcode == 2)
      printesc(strprotector(1171));
    else
      printesc(strprotector(1172));
    break;

  case def:   /*:1209*/
    if (chrcode == 0)
      printesc(strprotector(1173));
    else if (chrcode == 1)
      printesc(strprotector(1174));
    else if (chrcode == 2)
      printesc(strprotector(1175));
    else
      printesc(strprotector(1176));
    break;
    /*1220:*/

  case let:   /*:1220*/
    if (chrcode != normal)
      printesc(strprotector(1191));
    else
      printesc(strprotector(1190));
    break;
    /*1223:*/
  ...
  case shorthanddef:
    ...
    case chardefcode:
      printesc(strprotector(1192));
      break;

    case mathchardefcode:
      printesc(strprotector(1193));
      break;

    case countdefcode:
      printesc(strprotector(1194));
      break;

    case dimendefcode:
      printesc(strprotector(1195));
      break;

    case skipdefcode:
      printesc(strprotector(1196));
      break;

    case muskipdefcode:
      printesc(strprotector(1197));
      break;

    default:
      printesc(strprotector(1198));
      break;
    }
    break;
  ...
  case defcode:
    if (chrcode == catcodebase)
      printesc(strprotector(426));
    else if (chrcode == mathcodebase)
      printesc(strprotector(430));
    else if (chrcode == lccodebase)
      printesc(strprotector(427));
    else if (chrcode == uccodebase)
      printesc(strprotector(428));
    else if (chrcode == sfcodebase)
      printesc(strprotector(429));
    else
      printesc(strprotector(488));
    break;
  ...
  case hyphdata:   /*:1251*/
    if (chrcode == 1)
      printesc(strprotector(952));
    else
      printesc(strprotector(940));
    break;
    /*1255:*/

  case assignfontint:   /*:1255*/
    if (chrcode == 0)
      printesc(strprotector(1216));
    else
      printesc(strprotector(1217));
    break;
    /*1261:*/

  case setfont:
    print(strprotector(1225));
    slowprint(fontname[chrcode - fontbase]);
    if (fontsize[chrcode - fontbase] != fontdsize[chrcode - fontbase]) {
      print(strprotector(267));
      printscaled(fontsize[chrcode - fontbase]);
      print(strprotector(268));
    }
    break;
  ...
  case instream:   /*:1273*/
    if (chrcode == 0)
      printesc(strprotector(1228));
    else
      printesc(strprotector(1227));
    break;
    /*1278:*/

  case message:   /*:1278*/
    if (chrcode == 0)
      printesc(strprotector(1229));
    else
      printesc(strprotector(1230));
    break;
    /*1287:*/

  case caseshift:   /*:1287*/
    if (chrcode == lccodebase)
      printesc(strprotector(1236));
    else
      printesc(strprotector(1237));
    break;
    /*1292:*/
  ...
  case xray:   /*:1292*/
    ...
    case showboxcode:
      printesc(strprotector(1239));
      break;

    case showthecode:
      printesc(strprotector(1240));
      break;

    case showlists:
      printesc(strprotector(1241));
      break;

    default:
      printesc(strprotector(1238));
      break;
    }
    break;
    /*1295:*/
  ...
  case undefinedcs:
    print(strprotector(1248));
    break;

  case call:
    print(strprotector(1249));
    break;

  case longcall:
    printesc(strprotector(1250));
    break;

  case outercall:
    printesc(strprotector(1251));
    break;
  ...
  case extension:
    ...
    case opennode:
      printesc(strprotector(1284));
      break;

    case writenode:
      printesc(strprotector(603));
      break;

    case closenode:
      printesc(strprotector(1285));
      break;
    ...
    default:
      print(strprotector(1289));
      break;
    }/*:1346*/
    break;

  default:
    print(strprotector(577));
    break;
  }
}
...
223:*/
  ...
  if (n < localbase) {   /*229:*/
	...
	printspec(*fooequiv(n), strprotector(350));
      return;
    }
    if (n < muskipbase) {
      printesc(strprotector(407));
...
/* p2c: tex1.p: Warning: Symbol 'HASHISFULL' is not defined [221] */
	      overflow(strprotector(514), hashsize);
	    }
	    hashused--;
...
Static void newsavelevel(groupcode c)
{
  ...
  *foosaveindex(saveptr) = curboundary;
  if (curlevel == maxquarterword)
    overflow(strprotector(552), maxquarterword - minquarterword);
  curboundary = saveptr;
  curlevel++;
  saveptr++;
  curgroup = c;
...
Static void unsave(void)
{
  ...
  while (true) {
	...
	if (tracingrestores > 0)   /*_ENDSTAT*/
	  restoretrace(p, strprotector(554));
      } else {
	eqdestroy(eqtb[p - activebase]);
	eqtb[p - activebase] = savestack[saveptr];   /*_STAT*/
	if (tracingrestores > 0)   /*_ENDSTAT*/
	  restoretrace(p, strprotector(555));
      }
      continue;
    }
...
Static void preparemag(void)
{
  if (magset > 0 && mag != magset) {
    printnl(258);
    print(strprotector(557));
    printint(mag);
    print(strprotector(558));
    printnl(strprotector(559));
    helpptr = 2;
    helpline[1] = strprotector(560);
    helpline[0] = strprotector(561);
    interror(magset);
    geqworddefine(intbase + magcode, magset);
  }
  if (mag <= 0 || mag > 32768L) {
    printnl(258);
    print(strprotector(562));
    helpptr = 1;
    helpline[0] = strprotector(563);
    interror(mag);
...
/* p2c: tex1.p: Warning: Symbol 'TERMINALINPUT' is not defined [221] */
	      if (baseptr == 0)
		printnl(strprotector(584));
	      else
		printnl(strprotector(585));
	    } else {
	      printnl(strprotector(586));
	      if (name == 17)
		printchar(tobar("*"));
	      else
		printint(name - 1);
...
/* p2c: tex1.p: Warning: Symbol 'SETTRICKCOUNT' is not defined [221] */
	  ...
	  case parameter:
	    printnl(strprotector(588));
	    break;

	  case utemplate:
	  case vtemplate:
	    printnl(strprotector(589));
	    break;

	  case backedup:
	    if (loc == 0)
	      printnl(strprotector(590));
	    else
	      printnl(strprotector(591));
	    break;

	  case inserted:
	    printnl(strprotector(592));
	    break;
	  ...
	  case outputtext:
	    printnl(strprotector(593));
	    break;

	  case everypartext:
	    printnl(strprotector(594));
	    break;

	  case everymathtext:
	    printnl(strprotector(595));
	    break;

	  case everydisplaytext:
	    printnl(strprotector(596));
	    break;

	  case everyhboxtext:
	    printnl(strprotector(597));
	    break;

	  case everyvboxtext:
	    printnl(strprotector(598));
	    break;

	  case everyjobtext:
	    printnl(strprotector(599));
	    break;

	  case everycrtext:
	    printnl(strprotector(600));
	    break;

	  case marktext:
	    printnl(strprotector(601));
	    break;

	  case writetext:
	    printnl(strprotector(602));
	    break;
...
{
  if (inopen == maxinopen)
    overflow(strprotector(605), maxinopen);
  if (first == bufsize)
    overflow(strprotector(272), bufsize);
  inopen++;
  if (inputptr > maxinstack) {
...
/* p2c: tex1.p: Warning: Symbol 'BACKLIST' is not defined [221] */
  ...
  if (scannerstatus > skipping) {   /*338:*/
    ...
    if (curcs == 0) {
      printnl(258);
      print(strprotector(613));
    } else {
      curcs = 0;
      printnl(258);
      print(strprotector(614));
    }
    print(strprotector(615));   /*339:*/
    p = getavail();
    switch (scannerstatus) {

    case defining:
      print(strprotector(580));
      *fooinfo(p) = rightbracetoken + tobar("}");
      break;

    case matching:
      print(strprotector(621));
      *fooinfo(p) = partoken;
      longstate = outercall;
      break;
...
/* p2c: tex1.p: Warning: Symbol 'INSLIST' is not defined [221] */
    print(strprotector(616));
    sprintcs(warningindex);
    helpptr = 4;
    helpline[3] = strprotector(617);
    helpline[2] = strprotector(618);
    helpline[1] = strprotector(619);
    helpline[0] = strprotector(620);
    error();
  } else {
    printnl(258);
    print(strprotector(607));
    printcmdchr(iftest, curif);
    print(strprotector(608));
    printint(skipline);
    helpptr = 3;
    helpline[2] = strprotector(609);
    helpline[1] = strprotector(610);
    helpline[0] = strprotector(611);
    if (curcs != 0)
      curcs = 0;
    else
      helpline[2] = strprotector(612);
...
/* p2c: tex1.p: Warning: Symbol 'HEXTOCURCHR' is not defined [221] */
    ...
    case newline + invalidchar:   /*346:*/
      printnl(258);
      print(strprotector(622));
      helpptr = 2;
      helpline[1] = strprotector(623);
      helpline[0] = strprotector(624);
      deletionsallowed = false;
      error();
      deletionsallowed = true;
      goto _Lrestart;
...
_Llabcontinue:
	...
	if (s == 0) {   /*398:*/
	  printnl(258);
	  print(strprotector(659));
	  sprintcs(warningindex);
	  print(strprotector(660));
	  helpptr = 4;
	  helpline[3] = strprotector(661);
	  helpline[2] = strprotector(662);
	  helpline[1] = strprotector(663);
	  helpline[0] = strprotector(664);
	  error();
	  goto _Lexit;
	}
	/*:398*/
...
_Ldone:
	...
	if (longstate != longcall) {   /*396:*/
	  if (longstate == call) {
	    runaway();
	    printnl(258);
	    print(strprotector(654));
	    sprintcs(warningindex);
	    print(strprotector(655));
	    helpptr = 3;
	    helpline[2] = strprotector(656);
	    helpline[1] = strprotector(657);
	    helpline[0] = strprotector(658);
	    backerror();
	  }
...
/* p2c: tex1.p: Warning: Symbol 'STORENEWTOKEN' is not defined [221] */
	  ...
	  backinput();
	  printnl(258);
	  print(strprotector(646));
	  sprintcs(warningindex);
	  print(strprotector(647));
	  helpptr = 6;
	  helpline[5] = strprotector(648);
	  helpline[4] = strprotector(649);
	  helpline[3] = strprotector(650);
	  helpline[2] = strprotector(651);
	  helpline[1] = strprotector(652);
	  helpline[0] = strprotector(653);
	  alignstate++;
	  longstate = call;
	  curtok = partoken;
	  inserror();
...
/* p2c: tex1.p: Warning: Symbol 'FREEAVAIL' is not defined [221] */
  ...
  if (n > 0) {   /*:390*/
	...
	overflow(strprotector(645), paramsize);
    }
    for (m = 0; m < n; m++)
...
/* p2c: tex1.p: Warning: Symbol 'STORENEWTOKEN' is not defined [221] */
	...
	printnl(258);
	print(strprotector(634));
	printesc(strprotector(516));
	print(strprotector(635));
	helpptr = 2;
	helpline[1] = strprotector(636);
	helpline[0] = strprotector(637);
	backerror();
      }
      /*:373*/
      /*374:*/
	...
	if (iflimit == ifcode)
	  ...
	  printnl(258);
	  print(strprotector(784));
	  printcmdchr(fiorelse, curchr);
	  helpptr = 1;
	  helpline[0] = strprotector(785);
	  error();
	}
      } else {   /*:510*/
    ...
    default:
      printnl(258);
      print(strprotector(628));
      helpptr = 5;
      helpline[4] = strprotector(629);
      helpline[3] = strprotector(630);
      helpline[2] = strprotector(631);
      helpline[1] = strprotector(632);
      helpline[0] = strprotector(633);
      error();   /*:370*/
      break;
    }
  }  /*:367*/
...
{  /*404:*/
  ...
  if (curcmd == leftbrace)
    return;
  printnl(258);
  print(strprotector(666));
  helpptr = 4;
  helpline[3] = strprotector(667);
  helpline[2] = strprotector(668);
  helpline[1] = strprotector(669);
  helpline[0] = strprotector(670);
  backerror();
  ...
  *foolink(p) = 0;
  ...
  while (k < strstart[s + 1]) {
    ...
    if ((curcs == 0) & ((curchr == strpool[k]) |
			(curchr == strpool[k] - tobar("a") + tobar("A")))) {
      storenewtoken(curtok);
/* p2c: tex1.p: Warning: Symbol 'STORENEWTOKEN' is not defined [221] */
      k++;
      continue;
...
{
  printnl(258);
  print(strprotector(671));
  helpptr = 1;
  helpline[0] = strprotector(672);
  error();
}
...
Static void scaneightbitint(void)
{
  ...
  if ((unsigned long)curval <= 255)
    return;
  printnl(258);
  print(strprotector(696));
  helpptr = 2;
  helpline[1] = strprotector(697);
  helpline[0] = strprotector(698);
  interror(curval);
  curval = 0;
}
...
Static void scancharnum(void)
{
  ...
  if ((unsigned long)curval <= 255)
    return;
  printnl(258);
  print(strprotector(699));
  helpptr = 2;
  helpline[1] = strprotector(700);
  helpline[0] = strprotector(698);
  interror(curval);
  curval = 0;
}
...
Static void scanfourbitint(void)
{
  ...
  if ((unsigned long)curval <= 15)
    return;
  printnl(258);
  print(strprotector(701));
  helpptr = 2;
  helpline[1] = strprotector(702);
  helpline[0] = strprotector(698);
  interror(curval);
  curval = 0;
}
...
Static void scanfifteenbitint(void)
{
  ...
  if ((unsigned long)curval <= 32767)
    return;
  printnl(258);
  print(strprotector(703));
  helpptr = 2;
  helpline[1] = strprotector(704);
  helpline[0] = strprotector(698);
  interror(curval);
  curval = 0;
}
...
Static void scantwentysevenbitint(void)
{
  ...
  if ((unsigned long)curval <= 134217727L)
    return;
  printnl(258);
  print(strprotector(705));
  helpptr = 2;
  helpline[1] = strprotector(706);
  helpline[0] = strprotector(698);
  interror(curval);
  curval = 0;
}
...
{   /*406:*/
  ...
  if (curcmd == deffont)
    ...
    printnl(258);
    print(strprotector(817));
    helpptr = 2;
    helpline[1] = strprotector(818);
    helpline[0] = strprotector(819);
    backerror();
    f = nullfont;
  }
  curval = f;
...
Static void findfontdimen(boolean writing)
{
  ...
  if (n <= 0)
	...
	do {
	  if (fmemptr == fontmemsize)
	    overflow(strprotector(824), fontmemsize);
  ...
  if (curval != fmemptr)   /*:579*/
    return;
  printnl(258);
  print(strprotector(265));
  printesc(*foofontidtext(f));
  print(strprotector(820));
  printint(fontparams[f - fontbase]);
  print(strprotector(821));
  helpptr = 2;
  helpline[1] = strprotector(822);
  helpline[0] = strprotector(823);
  error();
}
...
Static void scansomethinginternal(smallnumber level, boolean negative)
{
  ...
  case deffont:   /*415:*/
    if (level != tokval) {
      printnl(258);
      print(strprotector(673));
      helpptr = 3;
      helpline[2] = strprotector(674);
      helpline[1] = strprotector(675);
      helpline[0] = strprotector(676);
      backerror();
      curval = 0;
  ...
  case setaux:   /*418:*/
    if (labs(mode) != m) {
      printnl(258);
      print(strprotector(689));
      printcmdchr(setaux, m);
      helpptr = 4;
      helpline[3] = strprotector(690);
      helpline[2] = strprotector(691);
      helpline[1] = strprotector(692);
      helpline[0] = strprotector(693);
      error();
      if (level != tokval) {
	curval = 0;
	curvallevel = dimenval;
  ...
  default:
    printnl(258);
    print(strprotector(694));
    printcmdchr(curcmd, curchr);
    print(strprotector(695));
    printesc(strprotector(548));
    helpptr = 1;
    helpline[0] = strprotector(693);
    error();
...
Static void scanint(void)
{
  ...
  do {   /*406:*/
    ...
    if (curtok == othertoken + tobar("-")) {   /*:441*/
      negative = !negative;
      curtok = othertoken + tobar("+");
    }
  } while (curtok == othertoken + tobar("+"));
  if (curtok == alphatoken) {   /*442:*/
    ...
    if (curval > 255) {
      printnl(258);
      print(strprotector(707));
      helpptr = 2;
      helpline[1] = strprotector(708);
      helpline[0] = strprotector(709);
      curval = tobar("0");
      backerror();
    } else {   /*443:*/
      getxtoken();
	...
	if (OKsofar) {
	  printnl(258);
	  print(strprotector(710));
	  helpptr = 2;
	  helpline[1] = strprotector(711);
	  helpline[0] = strprotector(712);
	  error();
	  curval = infinity;
	  OKsofar = false;
	}
...
/* p2c: tex1.p: Warning: Symbol 'FREEAVAIL' is not defined [221] */
  ...
  if (inf) {   /*454:*/
    if (scankeyword(strprotector(325))) {   /*:454*/
      curorder = fil;
      while (scankeyword(tobar("l"))) {
	if (curorder != filll) {
	  curorder++;
	  continue;
	}
	printnl(258);
	print(strprotector(714));
	print(strprotector(715));
	helpptr = 1;
	helpline[0] = strprotector(716);
	error();
      }
  ...
  if (mu)
    goto _Lnotfound;
  if (scankeyword(strprotector(717)))   /*443:*/
    v = *fooquad(curfont);   /*558:*/
    /*:558*/
  else if (scankeyword(strprotector(718)))
...
_Lnotfound:   /*:455*/
  if (mu) {   /*456:*/
    if (scankeyword(strprotector(350)))
      goto _Lattachfraction_;
    else {   /*:456*/
      printnl(258);
      print(strprotector(714));
      print(strprotector(719));
      helpptr = 4;
      helpline[3] = strprotector(720);
      helpline[2] = strprotector(721);
      helpline[1] = strprotector(722);
      helpline[0] = strprotector(723);
      error();
      goto _Lattachfraction_;
    }
  }
  if (scankeyword(strprotector(713))) {   /*457:*/
    preparemag();
    if (mag != 1000) {
...
/* p2c: tex1.p: Note: Using % for possibly-negative arguments [317] */
  ...
  /*:457*/
  if (scankeyword(strprotector(268)))   /*458:*/
    goto _Lattachfraction_;
  if (scankeyword(strprotector(724))) {
    num = 7227;
    denom = 100;
  } else if (scankeyword(strprotector(725))) {
    num = 12;
    denom = 1;
  } else if (scankeyword(strprotector(726))) {
    num = 7227;
    denom = 254;
  } else if (scankeyword(strprotector(727))) {
    num = 7227;
    denom = 2540;
  } else if (scankeyword(strprotector(728))) {
    num = 7227;
    denom = 7200;
  } else if (scankeyword(strprotector(729))) {
    num = 1238;
    denom = 1157;
  } else if (scankeyword(strprotector(730))) {
    num = 14856;
    denom = 1157;
  } else if (scankeyword(strprotector(731)))
    goto _Ldone;
  else {
    printnl(258);
    print(strprotector(714));
    print(strprotector(732));
    helpptr = 6;
    helpline[5] = strprotector(733);
    helpline[4] = strprotector(734);
    helpline[3] = strprotector(735);
    helpline[2] = strprotector(721);
    helpline[1] = strprotector(722);
    helpline[0] = strprotector(723);
    error();
...
_Lattachsign_:
  if (aritherror || labs(curval) >= 1073741824L) {   /*460:*/
    printnl(258);
    print(strprotector(736));
    helpptr = 2;
    helpline[1] = strprotector(737);
    helpline[0] = strprotector(738);
    error();
    curval = maxdimen;
    aritherror = false;
  }
...
Static void scanglue(smallnumber level)
{
  ...
  q = newspec(zeroglue);
  *foowidth(q) = curval;
  if (scankeyword(strprotector(739))) {
    scandimen(mu, true, false);
    *foostretch(q) = curval;
    *foostretchorder(q) = curorder;
  }
  if (scankeyword(strprotector(740))) {
    scandimen(mu, true, false);
    *fooshrink(q) = curval;
...
_Lreswitch:
  if (scankeyword(strprotector(741))) {
    scannormaldimen();
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
    *foowidth(q) = curval;
    goto _Lreswitch;
  }
  if (scankeyword(strprotector(742))) {
    scannormaldimen();
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
    *fooheight(q) = curval;
    goto _Lreswitch;
  }
  if (!scankeyword(strprotector(743)))
    return q;
  scannormaldimen();
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
  *foodepth(q) = curval;
...
/* p2c: tex1.p: Warning: Symbol 'FASTSTORENEWTOKEN' is not defined [221] */
    ...
    case glueval:
      printspec(curval, strprotector(268));
      deleteglueref(curval);
      break;

    case muval:
      printspec(curval, strprotector(350));
      deleteglueref(curval);
      break;
    }
    selector = oldsetting;
...
/* p2c: tex1.p: Warning: Symbol 'STORENEWTOKEN' is not defined [221] */
	...
	if (t == zerotoken + 9) {
	  printnl(258);
	  print(strprotector(752));
	  helpptr = 1;
	  helpline[0] = strprotector(753);
	  error();
	} else {
	  t++;
	  if (curtok != t) {
	    printnl(258);
	    print(strprotector(754));
	    helpptr = 2;
	    helpline[1] = strprotector(755);
	    helpline[0] = strprotector(756);
	    backerror();
	  }
	  curtok = s;
	}
...
/* p2c: tex1.p: Warning: Symbol 'STORENEWTOKEN' is not defined [221] */
    if (curcmd == rightbrace) {   /*475:*/
      ...
      alignstate++;
      helpptr = 2;
      helpline[1] = strprotector(750);
      helpline[0] = strprotector(751);
      error();
      goto _Lfound;
    }
    /*:475*/
...
_Ldone2:
	...
	if (curcmd != macparam) {
	  if (curtok <= zerotoken || curtok > t) {
	    printnl(258);
	    print(strprotector(757));
	    sprintcs(warningindex);
	    helpptr = 3;
	    helpline[2] = strprotector(758);
	    helpline[1] = strprotector(759);
	    helpline[0] = strprotector(760);
	    backerror();
	    curtok = s;
	  } else
	    curtok = outparamtoken - tobar("0") + curchr;
	}
      }
      /*:479*/
    }
...
/* p2c: tex1.p: Warning: Symbol 'STORENEWTOKEN' is not defined [221] */
  ...
  do {   /*483:*/
	...
	if (alignstate != 1000000L) {
	  runaway();
	  printnl(258);
	  print(strprotector(762));
	  printesc(strprotector(545));
	  helpptr = 1;
	  helpline[0] = strprotector(763);
	  alignstate = 1000000L;
	  error();
	}
      }
...
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
    ...
    if ((curtok >= othertoken + tobar("<")) & (curtok <= othertoken + tobar(">")))
      r = curtok - othertoken;
    else {
      printnl(258);
      print(strprotector(788));
      printcmdchr(iftest, thisif);
      helpptr = 1;
      helpline[0] = strprotector(789);
      backerror();
      r = tobar("=");
    }
    if (thisif == ifintcode)
...
508:*/
  ...
  if (tracingcommands > 1) {   /*502:*/
    begindiagnostic();
    if (b)
      print(strprotector(786));
    else
      print(strprotector(787));
    enddiagnostic(false);
  }
  /*:502*/
  if (b) {
...
/* p2c: tex1.p: Warning: Symbol 'CURLENGTH' is not defined [221] */
      extdelimiter = 0;
    } else if (c == tobar(".") && extdelimiter == 0) {
...
Static void promptfilename(strnumber s, strnumber e)
{
  ...
  if (s == strprotector(791)) {
    printnl(258);
    print(strprotector(792));
  } else {
    printnl(258);
    print(strprotector(793));
  }
  printfilename(curname, curarea, curext);
  print(strprotector(794));
  if (e == strprotector(795))
    showcontext();
  printnl(strprotector(796));
  print(s);
  if (interaction < scrollmode)
    fatalerror(strprotector(797));
  print(strprotector(578));
  terminput();   /*531:*/
  beginname();
  k = first;
  while (buffer[k] == tobar(" ") && k < last)
    k++;
  while (true) {
    if (k == last)
      goto _Ldone;
...
Static void openlogfile(void)
{
  ...
  oldsetting = selector;
  if (jobname == 0)
    jobname = strprotector(798);
  packjobname(strprotector(799));
  while (!aopenout(&logfile)) {   /*535:*/
    selector = termonly;
    promptfilename(strprotector(801), strprotector(799));
  }
  /*:535*/
...
/* p2c: tex1.p: Warning: Symbol 'PACKCURNAME' is not defined [221] */
  while (true) {
    ...
    if (aopenin(&curfile))
      goto _Ldone;
    if (curarea == strprotector(266)) {
      packfilename(curname, 261, curext);
      if (aopenin(&curfile))
	goto _Ldone;
    }
    endfilereading();
    promptfilename(strprotector(791), strprotector(795));
  }
_Ldone:
				       ...
				       strnumber aire, long s)
{
  ...
  /*563:*/
  fileopened = false;
  if (aire == strprotector(266))
    packfilename(nom, 262, strprotector(811));
  else
    packfilename(nom, aire, strprotector(811));
  if (!bopenin(&tfmfile))
    goto _Lbadtfm_;
  fileopened = true;   /*:563*/
  /*565:*/
...
/* p2c: tex1.p: Warning: Symbol 'GETC' is not defined [221] */
  ...
  if (fontptr == fontmax || fmemptr + lf > fontmemsize) {   /*567:*/
    ...
    print(strprotector(812));
    helpptr = 4;
    helpline[3] = strprotector(813);
    helpline[2] = strprotector(814);
    helpline[1] = strprotector(815);
    helpline[0] = strprotector(816);
    error();
    goto _Ldone;
  }
  /*:567*/
...
_Lbadtfm_:   /*561:*/
  ...
  if (fileopened)
    print(strprotector(804));
  else
    print(strprotector(805));
  helpptr = 5;
  helpline[4] = strprotector(806);
  helpline[3] = strprotector(807);
  helpline[2] = strprotector(808);
  helpline[1] = strprotector(809);
  helpline[0] = strprotector(810);   /*:561*/
  error();
_Ldone:
  if (fileopened)
    bclose(&tfmfile);
...
Static void charwarning(internalfontnumber f, eightbits c)
{
  if (tracinglostchars <= 0)
    return;
  begindiagnostic();
  printnl(strprotector(825));
  print(c);
  print(strprotector(826));
...
{   /*1371:*/
  ...
  q = getavail();
  *fooinfo(q) = rightbracetoken + tobar("}");
  r = getavail();
  *foolink(q) = r;
...
/* p2c: tex1.p: Warning: Symbol 'INSLIST' is not defined [221] */
  ...
  q = getavail();
  *fooinfo(q) = leftbracetoken + tobar("{");
  inslist(q);
/* p2c: tex1.p: Warning: Symbol 'INSLIST' is not defined [221] */
  oldmode = mode;
  mode = 0;
  ...
  if (curtok != endwritetoken) {   /*1372:*/
    printnl(258);
    print(strprotector(1296));
    helpptr = 2;
    helpline[1] = strprotector(1297);
    helpline[0] = strprotector(1011);
    error();
    do {
      gettoken();
...
/* p2c: tex1.p: Warning: Symbol 'PACKCURNAME' is not defined [221] */
	  while (!aopenout(&writefile[j]))
	    promptfilename(strprotector(1299), strprotector(795));
	  writeopen[j] = true;
	}
      }
    }
...
/* p2c: tex1.p: Warning: Symbol 'FFLUSH' is not defined [221] */
  ...
  if ((*fooheight(p) > maxdimen) | (*foodepth(p) > maxdimen) |
    ...
    printnl(258);
    print(strprotector(833));
    helpptr = 2;
    helpline[1] = strprotector(834);
    helpline[0] = strprotector(835);
    error();
    if (tracingoutput <= 0) {
      begindiagnostic();
      printnl(strprotector(836));
      showbox(p);
      enddiagnostic(true);
    }
    goto _Ldone;
...
/* p2c: tex1.p: Warning: Symbol 'FFLUSH' is not defined [221] */
  ...
  if (tracingstats <= 1)   /*_ENDSTAT*/
    return;
  /*:639*/
  print(strprotector(831));
  printint(varused);
  printchar(tobar("&"));
  printint(dynused);
  print(strprotector(832));
...
Static void scanspec(groupcode c, boolean threecodes)
{
  ...
  if (threecodes)
    s = *foosaved(0);
  if (scankeyword(strprotector(842)))
    speccode = exactly;
  else if (scankeyword(strprotector(843)))
    speccode = additional;
  else {
    speccode = additional;
    curval = 0;
...
659*/
    ...
    *foogluesign(r) = stretching;
	...
	if (lastbadness > hbadness) {
	  println();
	  if (lastbadness > 100)
	    printnl(strprotector(844));
	  else
	    printnl(strprotector(845));
	  print(strprotector(846));
    ...
    *foogluesign(r) = shrinking;
	...
	println();
	printnl(strprotector(852));
	printscaled(-x - totalshrink[0]);
	print(strprotector(853));
	goto _Lcommonending;
      }
    } else if (o == normal) {
...
_Lcommonending:   /*663:*/
  if (outputactive)
    print(strprotector(847));
  else {
    if (packbeginline != 0) {
      if (packbeginline > 0)
	print(strprotector(848));
      else
	print(strprotector(849));
      printint(labs(packbeginline));
      print(strprotector(850));
    } else
      print(strprotector(851));
    printint(line);
  }
  println();
...
659*/
    ...
    *foogluesign(r) = shrinking;
	...
	println();
	printnl(strprotector(857));
	printscaled(-x - totalshrink[0]);
	print(strprotector(858));
	goto _Lcommonending;
      }
    } else if (o == normal) {
...
Static void fetch(halfword a)
{
  ...
  if (curf == nullfont) {   /*723:*/
    ...
    printchar(tobar(" "));
    printint(*foofam(a));
    print(strprotector(884));
    print(curc - minquarterword);
    printchar(tobar(")"));
    helpptr = 4;
    helpline[3] = strprotector(885);
    helpline[2] = strprotector(886);
    helpline[1] = strprotector(887);
    helpline[0] = strprotector(888);
    error();
    curi = nullcharacter;
    *foomathtype(a) = empty;
    return;
...
Static void initalign(void)
{
  ...
  if (mode == mmode && (tail != head || incompleatnoad != 0)) {   /*:776*/
    ...
    print(strprotector(689));
    printesc(strprotector(531));
    print(strprotector(893));
    helpptr = 3;
    helpline[2] = strprotector(894);
    helpline[1] = strprotector(895);
    helpline[0] = strprotector(896);
    error();
    flushmath();
  }
  pushnest();   /*775:*/
  ...
  while (true) {   /*778:*/
    ...
    *foolink(p) = 0;
	...
	if (p == holdhead && curloop == 0 && curcmd == tabmark) {
	  ...
	  printnl(258);
	  print(strprotector(902));
	  helpptr = 3;
	  helpline[2] = strprotector(903);
	  helpline[1] = strprotector(904);
	  helpline[0] = strprotector(905);
	  backerror();
	  goto _Ldone1;
	}
      } else {
...
_Llabcontinue:
	...
	goto _Ldone2;
      if (curcmd == macparam) {
	printnl(258);
	print(strprotector(906));
	helpptr = 3;
	helpline[2] = strprotector(903);
	helpline[1] = strprotector(904);
	helpline[0] = strprotector(907);
	error();
	goto _Llabcontinue;
      }
      *foolink(p) = getavail();
	...
	*fooinfo(q) = *fooinfo(r);
	r = *foolink(r);
      ...
      *foolink(p) = newglue(*fooglueptr(curloop));
      ...
      printnl(258);
      print(strprotector(909));
      printesc(strprotector(898));
      helpptr = 3;
      helpline[2] = strprotector(910);
      helpline[1] = strprotector(911);
      helpline[0] = strprotector(912);
...
Static void finalign(void)
{
  ...
  if (curgroup != aligngroup)
    confusion(strprotector(914));
  unsave();
  if (curgroup != aligngroup)
    confusion(strprotector(915));
  unsave();
...
/* p2c: tex1.p: Warning: Incompatible types or sizes [167] */
  ...
  if (mode == mmode) {   /*1206:*/
    ...
    if (curcmd != mathshift) {   /*1207:*/
      printnl(258);
      print(strprotector(1169));
      helpptr = 2;
      helpline[1] = strprotector(894);
      helpline[0] = strprotector(895);
      backerror();
    } else {   /*1197:*/
      getxtoken();
      if (curcmd != mathshift) {
	printnl(258);
	print(strprotector(1165));
	helpptr = 2;
	helpline[1] = strprotector(1166);
	helpline[0] = strprotector(1167);
	backerror();
      }
    }
    /*:1207*/
...
Static halfword finiteshrink(halfword p)
{
  ...
  if (noshrinkerroryet) {
    ...
    printnl(258);
    print(strprotector(916));
    helpptr = 5;
    helpline[4] = strprotector(917);
    helpline[3] = strprotector(918);
    helpline[2] = strprotector(919);
    helpline[1] = strprotector(920);
    helpline[0] = strprotector(921);
    error();
  }
  q = newspec(p);
...
Static quarterword newtrieop(smallnumber d, smallnumber n, quarterword v)
{
  ...
  while (true) {
	...
	overflow(strprotector(948), trieopsize);
      u = trieused[curlang];
      if (u == maxquarterword)
	overflow(strprotector(949), maxquarterword - minquarterword);
      trieopptr++;
      u++;
      trieused[curlang] = u;
...
Static void firstfit(triepointer p)
{
  ...
  while (true) {
    ...
    if (triemax < h + 256) {   /*:954*/
      if (triesize <= h + 256)
	overflow(strprotector(950), triesize);
      do {
	triemax++;
...
/* p2c: tex1.p: Warning: Symbol 'SETCURLANG' is not defined [221] */
    ...
    while (true) {
      ...
      case letter:
      case otherchar:   /*962:*/
	if (digitsensed | (curchr < tobar("0")) | (curchr > tobar("9"))) {
	  if (curchr == tobar("."))
	    curchr = 0;
	  else {
	    ...
	    if (curchr == 0) {
	      printnl(258);
	      print(strprotector(956));
	      helpptr = 1;
	      helpline[0] = strprotector(955);
	      error();
	    }
	  }
	  if (k < 63) {
...
_Ldone:   /*:961*/
  ...
  printnl(258);
  print(strprotector(951));
  printesc(strprotector(952));
  helpptr = 1;
  helpline[0] = strprotector(953);
  error();
...
_Ldone5: ;
    ...
    if (secondpass) {  /*_STAT*/
      if (tracingparagraphs > 0)   /*_ENDSTAT*/
	printnl(strprotector(934));
      background[1] += emergencystretch;
      finalpass = true;
      continue;
    }
    if (tracingparagraphs > 0)   /*_ENDSTAT*/
      printnl(strprotector(933));
    threshold = tolerance;
    secondpass = true;
...
_Lreswitch:
	...
	if (*foolccode(curchr) == 0) {
	  printnl(258);
	  print(strprotector(944));
	  helpptr = 2;
	  helpline[1] = strprotector(945);
	  helpline[0] = strprotector(946);
	  error();
	} else if (n < 63) {
	  n++;
...
/* p2c: tex1.p: Warning: Symbol 'APPENDCHAR' is not defined [221] */
	...
	if (hyphcount == hyphsize)
	  overflow(strprotector(947), hyphsize);
	hyphcount++;
...
_Lnotfound:   /*:941*/
    ...
    default:
      ...
      print(strprotector(689));
      printesc(strprotector(940));
      print(strprotector(941));
      helpptr = 2;
      helpline[1] = strprotector(942);
      helpline[0] = strprotector(943);
      error();   /*:936*/
      break;
    }
  }
...
_Lupdateheights_:   /*976:*/
	...
	printnl(258);
	print(strprotector(960));
	helpptr = 4;
	helpline[3] = strprotector(961);
	helpline[2] = strprotector(962);
	helpline[1] = strprotector(963);
	helpline[0] = strprotector(921);
	error();
	r = newspec(q);
...
Static halfword vsplit(eightbits n, long h)
{
  ...
  if (*footype(v) != vlistnode) {   /*:978*/
    ...
    print(strprotector(266));
    printesc(strprotector(964));
    print(strprotector(965));
    printesc(strprotector(966));
    helpptr = 2;
    helpline[1] = strprotector(967);
    helpline[0] = strprotector(968);
    error();
    Result = 0;
    goto _Lexit;
  }
...
Static void printtotals(void)
{
  ...
  if (pagesofar[4] != 0) {
    ...
    print(strprotector(977));
  }
  if (pagesofar[5] != 0) {
    print(270);
    printscaled(pagesofar[5]);
    print(strprotector(978));
  }
  if (pageshrink != 0) {
    print(strprotector(326));
...
/* p2c: tex1.p: Warning: Symbol 'SETPAGESOFARZERO' is not defined [221] */
  ...
  if (tracingpages <= 0)   /*_ENDSTAT*/
    return;
  begindiagnostic();
  printnl(strprotector(986));
  printscaled(pagegoal);
  print(strprotector(987));
...
Static void ensurevbox(eightbits n)
{
  ...
  if (*footype(p) != hlistnode)
    return;
  printnl(258);
  print(strprotector(988));
  helpptr = 3;
  helpline[2] = strprotector(989);
  helpline[1] = strprotector(990);
  helpline[0] = strprotector(991);
  boxerror(n);
}
...
/* p2c: tex1.p: Warning: Symbol 'ADDTOKENREF' is not defined [221] */
  ...
  if (*foobox(255) != 0) {   /*:1015*/
    ...
    print(strprotector(266));
    printesc(strprotector(420));
    print(strprotector(1002));
    helpptr = 2;
    helpline[1] = strprotector(1003);
    helpline[0] = strprotector(991);
    boxerror(255);
  }
  insertpenalties = 0;
...
/* p2c: tex1.p: Warning: Symbol 'ADDTOKENREF' is not defined [221] */
  ...
  if (outputroutine != 0) {   /*1023:*/
    ...
    /*:1024*/
    printnl(258);
    print(strprotector(1004));
    printint(deadcycles);
    print(strprotector(1005));
    helpptr = 3;
    helpline[2] = strprotector(1006);
    helpline[1] = strprotector(1007);
    helpline[0] = strprotector(1008);
    error();
  }
...
/* p2c: tex1.p: Warning: Symbol 'PRECEDESBREAK' is not defined [221] */
	...
	*foobestinsptr(r) = 0;
	...
	if ((*fooshrinkorder(q) != normal) & (*fooshrink(q) != 0)) {
	  printnl(258);
	  print(strprotector(997));
	  printesc(strprotector(407));
	  printint(n);
	  helpptr = 3;
	  helpline[2] = strprotector(998);
	  helpline[1] = strprotector(999);
	  helpline[0] = strprotector(921);
	  error();
	}
      }
	...
	{   /*:1010*/
	  ...
	  *fooheight(r) += bestheightplusdepth;   /*_STAT*/
	  if (tracingpages > 0) {   /*1011:*/
	    begindiagnostic();
	    printnl(strprotector(1000));
	    printint(n);
	    print(strprotector(1001));
	    printscaled(w);
	    printchar(tobar(","));
...
_Lupdateheights_:   /*1004:*/
    if (*footype(p) == kernnode)
      ...
      if ((*fooshrinkorder(q) != normal) & (*fooshrink(q) != 0)) {
	printnl(258);
	print(strprotector(993));
	helpptr = 4;
	helpline[3] = strprotector(994);
	helpline[2] = strprotector(962);
	helpline[1] = strprotector(963);
	helpline[0] = strprotector(921);
	error();
...
Static void insertdollarsign(void)
{
  ...
  printnl(258);
  print(strprotector(1016));
  helpptr = 2;
  helpline[1] = strprotector(1017);
  helpline[0] = strprotector(1018);
  inserror();
}
...
Static void reportillegalcase(void)
{
  youcant();
  helpptr = 4;
  helpline[3] = strprotector(1020);
  helpline[2] = strprotector(1021);
  helpline[1] = strprotector(1022);
  helpline[0] = strprotector(1023);
  error();
}
...
Static void offsave(void)
{
  ...
  *foolink(temphead) = p;
  ...
  case mathshiftgroup:
    *fooinfo(p) = mathshifttoken + tobar("$");
    printchar(tobar("$"));
    break;
  ...
  case mathleftgroup:
    ...
    *foolink(p) = getavail();
    p = *foolink(p);
    *fooinfo(p) = othertoken + tobar(".");
    printesc(strprotector(1041));
    break;
...
/* p2c: tex1.p: Warning: Symbol 'INSLIST' is not defined [221] */
  helpptr = 5;
  helpline[4] = strprotector(1036);
  helpline[3] = strprotector(1037);
  helpline[2] = strprotector(1038);
  helpline[1] = strprotector(1039);
  helpline[0] = strprotector(1040);
  error();
}
...
Static void extrarightbrace(void)
{
  ...
  helpptr = 5;
  helpline[4] = strprotector(1048);
  helpline[3] = strprotector(1049);
  helpline[2] = strprotector(1050);
  helpline[1] = strprotector(1051);
  helpline[0] = strprotector(1052);
  error();
  alignstate++;
}
...
Static void boxend(long boxcontext)
{
  ...
  printnl(258);
  print(strprotector(1065));
  helpptr = 3;
  helpline[2] = strprotector(1066);
  helpline[1] = strprotector(1067);
  helpline[0] = strprotector(1068);
  backerror();
...
Static void beginbox(long boxcontext)
{
  ...
  case lastboxcode:   /*1080:*/
    ...
    if (labs(mode) == mmode) {
      youcant();
      helpptr = 1;
      helpline[0] = strprotector(1069);
      error();
    } else if (mode == vmode && head == tail) {
      youcant();
      helpptr = 2;
      helpline[1] = strprotector(1070);
      helpline[0] = strprotector(1071);
      error();
    } else {
      if (!ischarnode(tail)) {
...
_Ldone: ;
  ...
  case vsplitcode:   /*1082:*/
    ...
    if (!scankeyword(strprotector(842))) {
      printnl(258);
      print(strprotector(1072));
      helpptr = 2;
      helpline[1] = strprotector(1073);
      helpline[0] = strprotector(1074);
      error();
    }
    scannormaldimen();
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
...
{   /*404:*/
  ...
  printnl(258);
  print(strprotector(1075));
  helpptr = 3;
  helpline[2] = strprotector(1076);
  helpline[1] = strprotector(1077);
  helpline[0] = strprotector(1078);
  backerror();
}
...
Static void headforvmode(void)
{
  if (mode < 0) {
    ...
    print(strprotector(694));
    printesc(strprotector(532));
    print(strprotector(1081));
    helpptr = 2;
    helpline[1] = strprotector(1082);
    helpline[0] = strprotector(1083);
    error();
    return;
  }
  backinput();
...
Static void begininsertoradjust(void)
{
  if (curcmd == vadjust)
    ...
    if (curval == 255) {
      printnl(258);
      print(strprotector(1084));
      printesc(strprotector(343));
      printint(255);
      helpptr = 1;
      helpline[0] = strprotector(1085);
      error();
      curval = 0;
    }
  }
...
Static void deletelast(void)
{
  ...
  if (mode == vmode && tail == head) {   /*1106:*/
    if (curchr != gluenode || lastglue != maxhalfword) {
      ...
      helpptr = 2;
      helpline[1] = strprotector(1070);
      helpline[0] = strprotector(1086);
      if (curchr == kernnode)
	helpline[0] = strprotector(1087);
      else if (curchr != gluenode)
	helpline[0] = strprotector(1088);
      error();
    }
  }  /*:1106*/
  else {
...
Static void unpackage(void)
{
  ...
  if ((labs(mode) == mmode) |
    ...
    printnl(258);
    print(strprotector(1096));
    helpptr = 3;
    helpline[2] = strprotector(1097);
    helpline[1] = strprotector(1098);
    helpline[0] = strprotector(1099);
    error();
    goto _Lexit;
  }
  if (c == copycode)
  ...
  while (p != 0) {
	...
	if (*footype(p) != kernnode) {
	  if (*footype(p) != ligaturenode) {
	    printnl(258);
	    print(strprotector(1106));
	    helpptr = 1;
	    helpline[0] = strprotector(1107);
	    error();
	    begindiagnostic();
	    printnl(strprotector(1108));
	    showbox(p);
	    enddiagnostic(true);
	    flushnodelist(p);
	    *foolink(q) = 0;
...
_Ldone:   /*:1121*/
  ...
  case 2:   /*1120:*/
    if (n > 0 && labs(mode) == mmode) {
      printnl(258);
      print(strprotector(1100));
      printesc(strprotector(361));
      helpptr = 2;
      helpline[1] = strprotector(1101);
      helpline[0] = strprotector(1102);
      flushnodelist(p);
      n = 0;
      error();
    } else
    ...
    if (n <= maxquarterword)
      *fooreplacecount(tail) = n;
      ...
      printnl(258);
      print(strprotector(1103));
      helpptr = 2;
      helpline[1] = strprotector(1104);
      helpline[0] = strprotector(1105);
      error();
    }
    if (n > 0)
      tail = q;
...
Static void alignerror(void)
{
  if (labs(alignstate) > 2) {   /*1128:*/
    printnl(258);
    print(strprotector(1113));
    printcmdchr(curcmd, curchr);
    if (curtok == tabtoken + tobar("&")) {
      helpptr = 6;
      helpline[5] = strprotector(1114);
      helpline[4] = strprotector(1115);
      helpline[3] = strprotector(1116);
      helpline[2] = strprotector(1117);
      helpline[1] = strprotector(1118);
      helpline[0] = strprotector(1119);
    } else {
      helpptr = 5;
      helpline[4] = strprotector(1114);
      helpline[3] = strprotector(1120);
      helpline[2] = strprotector(1117);
      helpline[1] = strprotector(1118);
      helpline[0] = strprotector(1119);
    }
  ...
  if (alignstate < 0) {
    ...
    printnl(258);
    print(strprotector(1109));
    alignstate--;
    curtok = rightbracetoken + tobar("}");
  }
  helpptr = 3;
  helpline[2] = strprotector(1110);
  helpline[1] = strprotector(1111);
  helpline[0] = strprotector(1112);
  inserror();
}
...
Static void noalignerror(void)
{
  ...
  printesc(strprotector(538));
  helpptr = 2;
  helpline[1] = strprotector(1121);
  helpline[0] = strprotector(1122);
  error();
}
...
Static void mathlimitswitch(void)
{
  ...
  printnl(258);
  print(strprotector(1129));
  helpptr = 1;
  helpline[0] = strprotector(1130);
  error();
_Lexit: ;
}
...
Static void scandelimiter(halfword p, boolean r)
{
  ...
  if (curval < 0) {   /*1161:*/
    printnl(258);
    print(strprotector(1131));
    helpptr = 6;
    helpline[5] = strprotector(1132);
    helpline[4] = strprotector(1133);
    helpline[3] = strprotector(1134);
    helpline[2] = strprotector(1135);
    helpline[1] = strprotector(1136);
    helpline[0] = strprotector(1137);
    backerror();
    curval = 0;
  }
  /*:1161*/
...
Static void mathac(void)
{
  if (curcmd == accent) {   /*1166:*/
    printnl(258);
    print(strprotector(1138));
    printesc(strprotector(534));
    print(strprotector(1139));
    helpptr = 2;
    helpline[1] = strprotector(1140);
    helpline[0] = strprotector(1141);
    error();
  }
  /*:1166*/
...
/* p2c: tex1.p: Warning: Symbol 'TAILAPPEND' is not defined [221] */
    ...
    if (t != empty) {
      if (curcmd == supmark) {
	printnl(258);
	print(strprotector(1142));
	helpptr = 1;
	helpline[0] = strprotector(1143);
      } else {
	printnl(258);
	print(strprotector(1144));
	helpptr = 1;
	helpline[0] = strprotector(1145);
      }
      error();
    }
  }
...
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
    ...
    printnl(258);
    print(strprotector(1152));
    helpptr = 3;
    helpline[2] = strprotector(1153);
    helpline[1] = strprotector(1154);
    helpline[0] = strprotector(1155);
    error();
    return;
  }  /*:1183*/
...
Static void aftermath(void)
{
  ...
  if ((fontparams[*foofamfnt(textsize + 2) - fontbase] < totalmathsyparams) |
      ...
      (fontparams[*foofamfnt(scriptscriptsize + 2) - fontbase] <
       totalmathsyparams)) {
    printnl(258);
    print(strprotector(1157));
    helpptr = 3;
    helpline[2] = strprotector(1158);
    helpline[1] = strprotector(1159);
    helpline[0] = strprotector(1160);
    error();
    flushmath();
    danger = true;
	     ...
	     (fontparams[*foofamfnt(scriptscriptsize + 3) - fontbase] <
	      totalmathexparams)) {
    printnl(258);
    print(strprotector(1161));
    helpptr = 3;
    helpline[2] = strprotector(1162);
    helpline[1] = strprotector(1163);
    helpline[0] = strprotector(1164);
    error();
    flushmath();
    danger = true;
  }
...
_Lrestart:
  ...
  if (curcs != 0 && curcs <= frozencontrolsequence)
    return;
  printnl(258);
  print(strprotector(1183));
  helpptr = 5;
  helpline[4] = strprotector(1184);
  helpline[3] = strprotector(1185);
  helpline[2] = strprotector(1186);
  helpline[1] = strprotector(1187);
  helpline[0] = strprotector(1188);
  if (curcs == 0)
    backinput();
...
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
  ...
  if (aritherror) {
    printnl(258);
    print(strprotector(1206));
    helpptr = 2;
    helpline[1] = strprotector(1207);
    helpline[0] = strprotector(1208);
    error();
    goto _Lexit;
  }
  if (p < glueval) {
...
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
  ...
  printnl(258);
  print(strprotector(1212));
  helpptr = 1;
  helpline[0] = strprotector(1213);
  interror(curval);
}
...
/* p2c: tex1.p: Warning: Symbol 'DEFINE' is not defined [221] */
  ...
  scanfilename();   /*1258:*/
  nameinprogress = true;
  if (scankeyword(strprotector(1219))) {   /*1259:*/
    scannormaldimen();
/* p2c: tex1.p: Warning: Symbol 'SCANNORMALDIMEN' is not defined [221] */
    s = curval;
    if (s <= 0 || s >= 134217728L) {
      printnl(258);
      print(strprotector(1221));
      printscaled(s);
      print(strprotector(1222));
      helpptr = 2;
      helpline[1] = strprotector(1223);
      helpline[0] = strprotector(1224);
      error();
      s = unity * 10;
    }
  }  /*:1259*/
  else if (scankeyword(strprotector(1220))) {
    scanint();
    s = -curval;
...
Static void prefixedcommand(void)
{
  ...
  while (curcmd == prefix) {
    ...
    /*:1212*/
    printnl(258);
    print(strprotector(1178));
    printcmdchr(curcmd, curchr);
    printchar(tobar("'"));
    helpptr = 1;
    helpline[0] = strprotector(1179);
    backerror();
    goto _Lexit;
  }  /*1213:*/
  if (curcmd != def && (a & 3) != 0) {   /*:1213*/
    ...
    print(strprotector(694));
    printesc(strprotector(1170));
    print(strprotector(1180));
    printesc(strprotector(1171));
    print(strprotector(1181));
    printcmdchr(curcmd, curchr);
    printchar(tobar("'"));
    helpptr = 1;
    helpline[0] = strprotector(1182);
    error();
  }
  /*1214:*/
  if (globaldefs != 0) {
...
/* p2c: tex1.p: Warning: Symbol 'DEFINE' is not defined [221] */
  ...
  case readtocs:   /*:1225*/
    ...
    if (!scankeyword(strprotector(842))) {
      ...
      print(strprotector(1072));
      helpptr = 2;
      helpline[1] = strprotector(1199);
      helpline[0] = strprotector(1200);
      error();
    }
    getrtoken();
    p = curcs;
...
/* p2c: tex1.p: Warning: Symbol 'DEFINE' is not defined [221] */
  ...
  case defcode:   /*:1232*/
    ...
    if (curval < 0 && p < delcodebase || curval > n) {
      printnl(258);
      print(strprotector(1201));
      printint(curval);
      if (p < delcodebase)
	print(strprotector(1202));
      else
	print(strprotector(1203));
      printint(n);
      helpptr = 1;
      helpline[0] = strprotector(1204);
      error();
      curval = 0;
    }
    if (p < mathcodebase) {
...
/* p2c: tex1.p: Warning: Symbol 'GLOBAL' is not defined [221] */
    ...
    if (setboxallowed)
      ...
      printesc(strprotector(547));
      helpptr = 2;
      helpline[1] = strprotector(1210);
      helpline[0] = strprotector(1211);
      error();
    }
    break;
    /*1242:*/
...
/* p2c: tex1.p: Warning: Symbol 'FFLUSH' is not defined [221] */
  ...
  {   /*:1283*/
    ...
    if (errhelp != 0)
      ...
      helpptr = 1;
      helpline[0] = strprotector(1231);
    } else {
      if (interaction < errorstopmode)
	longhelpseen = true;
      helpptr = 4;
      helpline[3] = strprotector(1232);
      helpline[2] = strprotector(1233);
      helpline[1] = strprotector(1234);
      helpline[0] = strprotector(1235);
    }
    error();
    useerrhelp = false;
  }
...
Static void showwhatever(void)
{
  ...
  enddiagnostic(true);
  printnl(258);
  print(strprotector(1254));
  if (selector == termandlog) {
    if (tracingonline <= 0) {   /*:1298*/
      selector = termonly;
      print(strprotector(1255));
      selector = termandlog;
    }
  }
_Lcommonending:
  if (interaction < errorstopmode) {
    ...
    helpptr = 3;
    helpline[2] = strprotector(1242);
    helpline[1] = strprotector(1243);
    helpline[0] = strprotector(1244);
  } else {
    helpptr = 5;
    helpline[4] = strprotector(1242);
    helpline[3] = strprotector(1243);
    helpline[2] = strprotector(1244);
    helpline[1] = strprotector(1245);
    helpline[0] = strprotector(1246);
  }
  error();
}
...
{  /*1304:*/
  ...
  if (saveptr != 0) {
    printnl(258);
    print(strprotector(1257));
    helpptr = 1;
    helpline[0] = strprotector(1258);
    if (interaction == errorstopmode)
      interaction = scrollmode;
    if (logopened)   /*_DEBUG*/
      error();
...
/* p2c: tex1.p: Note: Using % for possibly-negative arguments [317] */
  ...
  while (!wopenout(&fmtfile))
    promptfilename(strprotector(1272), formatextension);
  printnl(strprotector(1273));
  slowprint(wmakenamestring(&fmtfile));
  flushstring();
/* p2c: tex1.p: Warning: Symbol 'FLUSHSTRING' is not defined [221] */
  printnl(strprotector(266));   /*:1328*/
...
/* p2c: tex1.p: Warning: Symbol 'PPUT' is not defined [221] */
  println();
  printint(x);
  print(strprotector(1260));
  printint(varused);
  printchar(tobar("&"));   /*:1311*/
...
/* p2c: tex1.p: Warning: Symbol 'PPUT' is not defined [221] */
  ...
  println();
  printint(fmemptr - 7);
  print(strprotector(1262));
  printint(fontptr - fontbase);
  print(strprotector(1263));
...
/* p2c: tex1.p: Warning: Symbol 'PPUT' is not defined [221] */
  ...
  printnl(strprotector(1266));
  printint(triemax);
  print(strprotector(1267));
  printint(trieopptr);
  print(strprotector(1268));
  if (trieopptr != 1)
    printchar(tobar("s"));
  print(strprotector(1269));
  printint(trieopsize);
  for (k = 255; k >= 0; k--) {   /*1326:*/
    if (trieused[k] > minquarterword) {   /*:1324*/
      printnl(strprotector(802));
      printint(trieused[k] - minquarterword);
      print(strprotector(1270));
      printint(k);
      pppfmtfile.int_ = k;
      pput(pppfmtfile);
/* p2c: tex1.p: Warning: Symbol 'PPUT' is not defined [221] */
...
Static void handlerightbrace(void)
{
  ...
  case bottomlevel:
    printnl(258);
    print(strprotector(1043));
    helpptr = 2;
    helpline[1] = strprotector(1044);
    helpline[0] = strprotector(1045);
    error();
    break;
...
/* p2c: tex1.p: Warning: Symbol 'TAILAPPEND' is not defined [221] */
  ...
  case outputgroup:   /*1026:*/
    ...
    {   /*:1027*/
      printnl(258);
      print(strprotector(1009));
      helpptr = 2;
      helpline[1] = strprotector(1010);
      helpline[0] = strprotector(1011);
      error();
      do {
	gettoken();
    ...
    if (*foobox(255) != 0) {   /*:1028*/
      printnl(258);
      print(strprotector(1012));
      printesc(strprotector(420));
      printint(255);
      helpptr = 3;
      helpline[2] = strprotector(1013);
      helpline[1] = strprotector(1014);
      helpline[0] = strprotector(1015);
      boxerror(255);
    }
    if (tail != head) {
...
/* p2c: tex1.p: Warning: Symbol 'DVIOUT' is not defined [221] */
    ...
    if (dviptr > 0)   /*:599*/
      writedvi(0, dviptr - 1);
    printnl(strprotector(838));
    slowprint(outputfilename);
    print(strprotector(300));
    printint(totalpages);
    print(strprotector(839));
    if (totalpages != 1)
      printchar(tobar("s"));
    print(strprotector(840));
    printint(dvioffset + dviptr);
    print(strprotector(841));
    bclose(&dvifile);
  }
  if (!logopened)
    return;
...
Static void finalcleanup(void)
{
  ...
  while (openparens > 0) {
    print(strprotector(1275));
    openparens--;
  }
  if (curlevel > levelone) {
    printnl(tobar("("));
    printesc(strprotector(1276));
    print(strprotector(1277));
  ...
  while (condptr != 0) {
    printnl(tobar("("));
    printesc(strprotector(1276));
    print(strprotector(1278));
    printcmdchr(iftest, curif);
    if (ifline != 0) {
      print(strprotector(1279));
      printint(ifline);
    }
    print(strprotector(1280));
...
Static void initprim(void)
{
  ...
  primitive(strprotector(388), assignglue, gluebase);
  ...
  primitive(strprotector(393), assignglue,
	    gluebase + abovedisplayshortskipcode);
  primitive(strprotector(394), assignglue,
  ...
  /*:226*/
  /*230:*/
  primitive(strprotector(409), assigntoks, outputroutineloc);
  ...
  primitive(strprotector(417), assigntoks, errhelploc);   /*:230*/
  /*238:*/
  primitive(strprotector(431), assignint, intbase);
  ...
  /*:238*/
  /*248:*/
  primitive(strprotector(489), assigndimen, dimenbase);
  ...
  primitive(strprotector(499), assigndimen,
	    dimenbase + delimitershortfallcode);
  primitive(strprotector(500), assigndimen,
  ...
  /*:248*/
  /*265:*/
  primitive(tobar(" "), exspace, 0);
  primitive(tobar("/"), italcorr, 0);
  primitive(strprotector(519), accent, 0);
  ...
  *footext(frozenendgroup) = strprotector(527);
  ...
  primitive(strprotector(343), insert_, 0);
  primitive(strprotector(363), mark_, 0);
  ...
  *footext(frozenrelax) = strprotector(546);
  ...
  primitive(strprotector(547), setbox, 0);
  ...
  primitive(strprotector(551), vrule, 0);   /*:265*/
  /*334:*/
  primitive(strprotector(606), parend, 256);
  parloc = curval;
  partoken = cstokenflag + parloc;   /*:334*/
  /*376:*/
  primitive(strprotector(638), input, 0);
  primitive(strprotector(639), input, 1);   /*:376*/
  /*384:*/
  ...
  primitive(strprotector(644), topbotmark, splitbotmarkcode);   /*:384*/
  /*411:*/
  primitive(strprotector(487), register_, intval);
  primitive(strprotector(511), register_, dimenval);
  primitive(strprotector(407), register_, glueval);
  primitive(strprotector(408), register_, muval);   /*:411*/
  /*416:*/
  ...
  primitive(strprotector(688), lastitem, badnesscode);   /*:416*/
  /*468:*/
  ...
  primitive(strprotector(749), convert, jobnamecode);   /*:468*/
  /*487:*/
  ...
  *footext(frozencr) = strprotector(898);
  ...
  *footext(frozenendv) = strprotector(900);
  ...
  *fooeqtype(frozenendtemplate) = endtemplate;   /*:780*/
  ...
  primitive(strprotector(976), setpagedimen, 7);   /*:983*/
  /*1052:*/
  primitive(strprotector(1024), stop, 0);
  primitive(strprotector(1025), stop, 1);   /*:1052*/
  /*1058:*/
  ...
  primitive(strprotector(354), mkern, muglue);   /*:1058*/
  /*1071:*/
  ...
  primitive(strprotector(1064), leadership, xleaders);   /*:1071*/
  /*1088:*/
  primitive(strprotector(1079), startpar, 1);
  primitive(strprotector(1080), startpar, 0);   /*:1088*/
  /*1107:*/
  ...
  primitive(strprotector(1095), unvbox, copycode);   /*:1107*/
  /*1114:*/
  primitive(tobar("-"), discretionary, 1);
  primitive(strprotector(361), discretionary, 0);   /*:1114*/
  /*1141:*/
  primitive(strprotector(1126), eqno, 0);
  primitive(strprotector(1127), eqno, 1);   /*:1141*/
  /*1156:*/
  ...
  primitive(strprotector(879), limitswitch, nolimits);   /*:1156*/
  /*1169:*/
  ...
  primitive(strprotector(864), mathstyle, scriptscriptstyle);   /*:1169*/
  /*1178:*/
  ...
  *footext(frozenright) = strprotector(877);
  ...
  primitive(strprotector(1173), def, 0);
  primitive(strprotector(1174), def, 1);
  primitive(strprotector(1175), def, 2);
  primitive(strprotector(1176), def, 3);
  /*:1208*/
  /*1219:*/
  primitive(strprotector(1190), let, normal);
  primitive(strprotector(1191), let, normal + 1);   /*:1219*/
  /*1222:*/
  ...
  primitive(strprotector(1198), shorthanddef, toksdefcode);   /*:1222*/
  /*1230:*/
  ...
  /*:1230*/
  /*1250:*/
  primitive(strprotector(940), hyphdata, 0);
  primitive(strprotector(952), hyphdata, 1);   /*:1250*/
  /*1254:*/
  primitive(strprotector(1216), assignfontint, 0);
  primitive(strprotector(1217), assignfontint, 1);   /*:1254*/
  /*1262:*/
  ...
  /*1272:*/
  primitive(strprotector(1227), instream, 1);
  primitive(strprotector(1228), instream, 0);   /*:1272*/
  /*1277:*/
  primitive(strprotector(1229), message, 0);
  primitive(strprotector(1230), message, 1);   /*:1277*/
  /*1286:*/
  primitive(strprotector(1236), caseshift, lccodebase);
  primitive(strprotector(1237), caseshift, uccodebase);   /*:1286*/
  /*1291:*/
  ...
  primitive(strprotector(1241), xray, showlists);   /*:1291*/
  /*1344:*/
  primitive(strprotector(1284), extension, opennode);
  primitive(strprotector(603), extension, writenode);
  writeloc = curval;
...
/* p2c: tex1.p: Warning: Symbol 'FFLUSH' is not defined [221] */
  ...
  limit = last;
  first = last + 1;   /*:331*/
  if ((formatident == 0) | (buffer[loc] == tobar("&"))) {
    if (formatident != 0)
      initialize();
    if (!openfmtfile())
      goto _Lfinalend;
    ...
    wclose(&fmtfile);
    while ((loc < limit) & (buffer[loc] == tobar(" ")))
      loc++;
  }
  if (endlinecharinactive) {
/* p2c: tex1.p:
